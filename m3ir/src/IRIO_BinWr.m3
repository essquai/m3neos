...
IMPORT M3Buf, M3ID, M3IR, M3IR_Asm, IRIO_Binary;
...
PROCEDURE OutI  (u: U;  i: INTEGER) =
  BEGIN
    IF (0 <= i) AND (i <= IRIO_Binary.LastRegular) THEN
      OutB (u, i);
    ELSIF (i < 0) THEN
      IF (-255 <= i) THEN
        i := -i;
        OutB (u, IRIO_Binary.NInt1);
        OutB (u, i);
      ELSIF (i >= -16_FFFF) THEN
        i := -i;
        OutB (u, IRIO_Binary.NInt2);
    ...
    ELSE (* i > 0 *)
      IF (i <= 255) THEN
        OutB (u, IRIO_Binary.Int1);
        OutB (u, i);
      ELSIF (i <= 16_FFFF) THEN
        OutB (u, IRIO_Binary.Int2);
        OutB (u, Word.And (i, 16_FF));
        OutB (u, Word.And (Word.RightShift (i, 8), 16_FF));
      ELSE
        AddBigInt (u, i);
...
PROCEDURE DumpInt (u: U;  VAR i: IntDesc) =
  CONST
    Tag4 = ARRAY BOOLEAN OF [0..255] { IRIO_Binary.NInt4, IRIO_Binary.Int4 };
    Tag8 = ARRAY BOOLEAN OF [0..255] { IRIO_Binary.NInt8, IRIO_Binary.Int8 };
...
PROCEDURE New (output: Wr.T): M3IR.T =
  VAR mbuf := M3Buf.New ();
  BEGIN
...
PROCEDURE set_error_handler (<*UNUSED*> u: U;
                             <*UNUSED*> p: M3IR_Asm.ErrorHandler) =
  BEGIN
