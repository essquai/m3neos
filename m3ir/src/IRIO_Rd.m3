...
IMPORT M3ID, M3IR, M3IR_Asm, Target, TInt, TFloat;
...
PROCEDURE Error (<*UNUSED*> VAR s: State;  a, b, c: TEXT := NIL) =
  ...
  VAR msg := Wr.EOL & "** ERROR in IRIO_Rd.Inhale: ";
  BEGIN
...
PROCEDURE Scan_id (VAR s: State): M3IR.Name =
  VAR buf: ARRAY [0..99] OF CHAR;  len := Scan_buf (s, buf);
  BEGIN
...
PROCEDURE Scan_type (VAR s: State): M3IR.Type =
  ...
  BEGIN
    IF types.get (name, val) THEN RETURN VAL (val, M3IR.Type) END;
...
PROCEDURE Scan_sign (VAR s: State): M3IR.Sign =
  ...
  BEGIN
    ...
    ELSIF (buf[0] = 'X') THEN  RETURN M3IR.Sign.Unknown;
    ELSIF (buf[0] = 'N') THEN  RETURN M3IR.Sign.Negative;
    ELSIF (buf[0] = 'P') THEN  RETURN M3IR.Sign.Positive;
