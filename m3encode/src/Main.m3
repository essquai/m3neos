...
IMPORT M3IR, IRIO_Rd, IRIO_Wr, IRIO_BinRd, IRIO_BinWr, MxConfig, Target;
...
PROCEDURE DoIt () RAISES {Rd.Failure, Wr.Failure, Alerted, OSError.E} =
  TYPE Inhale_t = PROCEDURE (rd: Rd.T;  cg: M3IR.T);
       OutNew_t = PROCEDURE (wr: Wr.T): M3IR.T;
  CONST Inhale = ARRAY OF Inhale_t {IRIO_BinRd.Inhale, IRIO_Rd.Inhale};
  CONST OutNew = ARRAY OF OutNew_t {IRIO_BinWr.New, IRIO_Wr.New};
  CONST inout_text = ARRAY OF TEXT {"-in-", "-out-"};
  CONST types_text = ARRAY OF TEXT {"binary", "ascii" };
  VAR arg : TEXT;
      inited := FALSE;
      rd_in := Stdio.stdin;
      wr_out := Stdio.stdout;
...
VAR arg0 := Params.Get(0);
  BEGIN
    ...
    IO.Put ("       " & arg0 & " -[in|out]-[binary|ascii] < in > out" & Wr.EOL);
    IO.Put ("       " & arg0 & " -[in|out]-[binary|ascii][:=file] < in > out" & Wr.EOL);
    IO.Put ("       example: " & arg0 & " -in-binary -out-ascii < in.bin > out.txt" & Wr.EOL);
    IO.Put ("       example: " & arg0 & " -in-binary=in.bin -out-ascii:out.txt" & Wr.EOL);
    Process.Exit (1);
  END Usage;
