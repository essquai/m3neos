...
IMPORT Text;
IMPORT IR, Host, Target, M3RT, Module;
...
PROCEDURE EmitAll (VAR proc_info: INTEGER) =
  VAR
    ...
    n_base      : INTEGER;
    n, total    : INTEGER;
    consts      : IR.Var := Module.GlobalData (is_const := TRUE);
  BEGIN
    proc_info := -1;
    ...
    IF (n > 0) THEN
      ...
      IR.Comment (n_base, TRUE, "procedure names");
      ...
      IR.Comment (base, TRUE, "procedure table");
      proc_info := base;
      ...
      WHILE (t # NIL) DO
        IF (t.cg_proc # NIL) AND (t.name # NIL) THEN
          IR.Init_chars (n_base + total, t.name, is_const := TRUE);
      ...
      WHILE (t # NIL) DO
        IF (t.cg_proc # NIL) THEN
          IF (t.name # NIL) THEN
            IR.Init_proc (n + M3RT.PI_proc, t.cg_proc, is_const := TRUE);
            IR.Init_var (n + M3RT.PI_name, consts, n_base+total, is_const := TRUE);
            INC (total, Target.Char.size * (Text.Length (t.name) + 1));
          END;
          INC (n, M3RT.PI_SIZE);
        END;
  ...
  BEGIN
    WHILE (t # NIL) DO
      IF (Host.nested_procs_first) THEN
        EmitBody (t.children);
        IF (t.name # NIL) THEN IR.Comment (-1, FALSE, t.name) END;
        t.gen_body ();
      ELSE
        IF (t.name # NIL) THEN IR.Comment (-1, FALSE, t.name) END;
        t.gen_body ();
