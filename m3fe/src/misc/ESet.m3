...
IMPORT M3, M3ID, IR, Value, Token, Scope, Scanner, M3Buf;
...
PROCEDURE Declare (t: T) =
  ...
  BEGIN
    ...
    WHILE (e # NIL) DO
      IF (e.except # NIL) THEN
        IR.Init_intt (n, Target.Integer.size, Exceptionz.UID (e.except), TRUE);
...
PROCEDURE GetAddress (t: T;  VAR base: IR.Var;  VAR offset: INTEGER) =
  BEGIN
    IF (t = NIL) THEN
      base := NIL;
      offset := 0;
...
PROCEDURE EmitNames (t: T) =
  ...
  BEGIN
    ...
    WHILE (e # NIL) DO
      IF (e.except # NIL) THEN
        IR.Declare_raises (M3ID.Add (Value.GlobalName (e.except)));
      END;
      e := e.next;
    END;
  END EmitNames;
