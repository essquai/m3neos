...
IMPORT Target, M3IR, M3, M3ID;

(*
This interface provides a single front-end specific, sometimes
quite thick, veneer over M3IR, M3IR_Ops and M3RT.
*)

TYPE (* see M3IR for the interpretation of these types *)
  Type        = M3IR.Type;
  MType       = M3IR.MType;
  IType       = M3IR.IType;
  RType       = M3IR.RType;
  ...
  Var         = M3IR.Var;
  Proc        = M3IR.Proc;
  Offset      = M3IR.BitOffset;
  Size        = M3IR.BitSize;
  Alignment   = M3IR.Alignment;
  (* NOTE: Notwithstanding the comment at M3IR.Alignment, alignments
           this interface are in *bits*. *)
  TypeUID     = M3IR.TypeUID;
  Label       = M3IR.Label;
  Frequency   = M3IR.Frequency;
  Cmp         = M3IR.CompareOp;
  Cvt         = M3IR.ConvertOp;
  AtomicOp    = M3IR.AtomicOp;
  MemoryOrder = M3IR.MemoryOrder;
...
CONST (* see M3IR for the interpretation of these values *)
  ...
  Maybe  : Frequency = M3IR.Maybe;
  Likely : Frequency = M3IR.Likely;
  Always : Frequency = M3IR.Always;
...
(* free "temps" (IR.Vars) that are marked busy. *)

(* NOTE: This naming is very confusing.  A IR.Var and IR.Val are different
...
(* NOTE: This naming is very confusing.  A IR.Var and IR.Val are different
         things.  But both are referred-to in many places as temporaries,
         temps, tmps, etc.  'Free' frees a IR.Val.  It claims to also free
         IR.Vars that the Val points to, but this is disabled. *)

PROCEDURE Store_temp (v: Val);
(* v := s0;  pop  -- v must have been created by "Pop_temp" *)

PROCEDURE ForceStacked (s: Size := 0); 
(* force s0 to be materialized on the M3IR stack.  If s0 is an L-value,
...
PROCEDURE Begin_clause (l: Label;  condition: BOOLEAN);
