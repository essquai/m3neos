...
PROCEDURE Declare_object (t, super: TypeUID;  brand: TEXT;  traced: BOOLEAN;
                           ...
                           <*UNUSED*> n_overrides: INTEGER; field_size: Size) =
  BEGIN
    cg.declare_object (t, super, brand, traced,
                       n_fields, n_methods, field_size);
  END Declare_object;

PROCEDURE Declare_method (n: Name;  signature: TypeUID;  <*UNUSED*> dfault: M3.Expr) =
  BEGIN
    cg.declare_method (n, signature);
  END Declare_method;

PROCEDURE Declare_override (<*UNUSED*> n: Name;  <*UNUSED*> dfault: M3.Expr) =
  BEGIN
...
(* TODO: It would be a lot cleaner and more consistent if a ValRec had a size
  ...
  BEGIN
    WITH x = stack [SCheck (1, "Force")] DO
      ...
      | VKind.Stacked =>
          (* value is already on the M3IR stack *)
...
(* REVIEW: This seems peculiar.  It converts any Word* type to its
           ...
           M3IR.i3), which claims to be the set of arithmetic types
...
(* PRE: (s0.A + o) is a byte multiple.
  ...
  BEGIN
    ...
    (* This uses a mix of cg.* calls, which only alter the M3IR stack,
       and CG calls, which manipulate the CG stack and M3IR stack,
       keeping them consistent.  This gets confusing.  The comments
       show the contents of both stacks, CG first.  Items on the M3IR
...
(* PRE: (s0.A + o) is a byte multiple.
  ...
  BEGIN
    ...
    (* This uses a mix of cg.* calls, which manipulate only the M3IR
       ...
       both stacks, CG first.  Items on the M3IR stack in parentheses
  ...
  BEGIN
    WITH xa = stack [SCheck (2, "Swap-a")],
      ...
      IF ((xa.kind = VKind.Stacked) OR (xa.kind = VKind.Pointer))
        ...
        (* Both values are on the M3IR stack => must swap them there. *)
...
(* Push host integer i on the M3IR stack, with target type Integer.
...
(* Push host integer i on the M3IR stack, with target type Word.
  No action on CG stack. *) 
  VAR val: Target.Int;  b := TInt.FromInt (i, val);
  BEGIN
    IF NOT b
...
(* push addr on the M3IR stack, with type Address.
   No action on CG stack. *) 
