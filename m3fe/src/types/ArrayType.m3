...
IMPORT M3, IR, Type, TypeRep, Error, Token;
...
(* Given "ADR(a)" and "index" on the stack, generate code to replace them
   ...
   IR 'ValRec's and may include a 'bits' expression. Generates no bounds
   checks. *)
  ...
  BEGIN
    ...
    ELSE (* We have an array with non-byte-aligned packed elements. *)
      ...
      ELSE
        bit_index := IR.Pop ();
        IR.Push (bit_index);
        IR.Load_intt (array_align);
        IR.Div (Target.Integer.cg_type, IR.Sign.Positive, IR.Sign.Positive);
        IR.Index_bytes (array_align);
        IR.Push (bit_index);
        IR.Load_intt (array_align);
        IR.Mod (Target.Integer.cg_type, IR.Sign.Positive, IR.Sign.Positive);
        IR.Index_bits ();
        IR.Free (bit_index);
      END;
    END;
...
(* FIXME: Cross-compile 32-bit host to 64-bit target. *)
    ...
    p.info.mem_type  := IR.Type.Struct;
...
PROCEDURE Compiler (p: P) =
  ...
  BEGIN
    ...
    IR.Declare_array (self, index, elt, p.total_size);
  END Compiler;
...
PROCEDURE GenInit (p: P;  zeroed: BOOLEAN) =
  VAR
    top   : IR.Label;
    cnt   : IR.Val;
    array := IR.Pop ();  (* capture the array's l-value *)
  BEGIN
    ...
    IR.Push (cnt);
    IR.Load_intt (p.n_elts);
    IR.If_compare (Target.Integer.cg_type, IR.Cmp.LT, top, IR.Likely);
