...
IMPORT M3, M3ID, IR, Type, TypeRep, Scope, Expr, Value, Token;
...
PROCEDURE Compiler (p: P) =
  ...
  BEGIN
    ...
    WHILE (o # NIL) DO
      ...
      INC (n);
    END;
    IR.Declare_record (Type.GlobalUID (p), p.recSize, n);
    o := fields;
...
PROCEDURE GenInit (p: P;  zeroed: BOOLEAN) =
  VAR
    ...
    ptr    := IR.Pop (); (* capture the record's address *)
  BEGIN
    WHILE (v # NIL) DO
      Field.Split (v, field);
      IF (field.dfault = NIL) THEN
        IF (Type.InitCost (field.type, zeroed) > 0) THEN
          IR.Push (ptr);
          IR.Boost_addr_alignment (p.align);
