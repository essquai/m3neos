...
IMPORT M3, M3ID, IR, Type, TypeRep, Scope, Expr, Host, TInt, UserProc;
...
PROCEDURE Check (p: P) =
  ...
  BEGIN
    ...
    p.info.mem_type  := IR.Type.Addr;
    p.info.stk_type  := IR.Type.Addr;
...
PROCEDURE Compiler (p: P) =
  ...
  BEGIN
    ...
    IR.Declare_object (Type.GlobalUID (p), Type.GlobalUID (p.superType),
  ...
  BEGIN
    WHILE (methods # NIL) DO
      ...
      IF (method.override) THEN
        (* skip *)
      ELSIF (declare) THEN
        IR.Declare_method (method.name, Type.GlobalUID (method.signature),
                           method.dfault);
      ELSE
  ...
  BEGIN
    WHILE (methods # NIL) DO
      ...
      IF (method.override) AND (declare) THEN
        IR.Declare_override (method.name, method.dfault);
      END;
      methods := methods.next;
    END;
  END GenOverrides;
...
PROCEDURE InitTypecell (t: Type.T;  offset, prev: INTEGER) =
  ...
  BEGIN
    ...
    IF (p.user_name # NIL) THEN
      name_offs := IR.EmitText (p.user_name, is_const := TRUE);
    END;

    (* generate my Type cell info *)
    IR.Init_intt   (offset + M3RT.TC_selfID, isz, Type.GlobalUID (p), FALSE);
    FOR i := FIRST (fp.byte) TO LAST (fp.byte) DO
      IR.Init_intt (offset + M3RT.TC_fp + i * 8, 8, fp.byte[i], FALSE);
    END;
    IR.Init_intt (offset + M3RT.TC_traced, 8, ORD (p.isTraced), FALSE);
    IR.Init_intt (offset + M3RT.TC_kind, 8, ORD (M3RT.TypeKind.Obj), FALSE);
    IR.Init_intt
    ...
    IF (type_map >= 0) THEN
      IR.Init_var (offset + M3RT.TC_type_map, consts, type_map, FALSE);
    END;
    IF (gc_map >= 0) THEN
      IR.Init_var (offset + M3RT.TC_gc_map, consts, gc_map, FALSE);
    END;
    IF (type_desc >= 0) THEN
      IR.Init_var (offset + M3RT.TC_type_desc, consts, type_desc, FALSE);
    END;
    IF (initProc # NIL) THEN
      IR.Init_proc (offset + M3RT.TC_initProc, initProc, FALSE);
    END;
    IF (brand >= 0) THEN
      IR.Init_var (offset + M3RT.TC_brand, consts, brand, FALSE);
    END;
    IF (p.user_name # NIL) THEN
      IR.Init_var (offset + M3RT.TC_name,  consts, name_offs, FALSE);
    END;
    IF (prev # 0) THEN
      IR.Init_var (offset + M3RT.TC_next,  globals, prev, FALSE);
    END;

    (* OBJECT specific extensions to the typecell *)
    IR.Init_intt (offset + M3RT.OTC_parentID, isz, super_id, FALSE);
    IF (linkProc # NIL) THEN
      IR.Init_proc (offset + M3RT.OTC_linkProc, linkProc, FALSE);
    END;
    IR.Init_intt (offset + M3RT.OTC_methodSize, isz,
                      p.methodSize DIV Target.Byte, FALSE);
    IF (defaults >= 0) THEN
      IR.Init_var (offset + M3RT.OTC_defaultMethods, consts, defaults, FALSE);
    END;
...
TYPE
  MethodValue = RECORD known: BOOLEAN;  proc: IR.Proc;  END;
  ...
  BEGIN
    ...
    FOR i := FIRST (m) TO LAST (m) DO
      VAR p := m[i].proc; BEGIN
        IF p # NIL THEN
          IR.Init_proc (offset + i * Target.Address.size, p, TRUE);
        END;
      END;
    END;
    RETURN offset;
...
PROCEDURE GenInitProc (p: P): IR.Proc =
  VAR
  ...
  BEGIN
    ...
    IR.Comment (-1, FALSE, name & " (ObjectType)");
    ...
    WHILE (v # NIL) DO
      ...
      IF (field.dfault = NIL) THEN
        IF Type.InitCost (field.type, TRUE) > 0 THEN
          IR.Push (ptr);
          IR.Boost_addr_alignment (p.fieldAlign);
      ...
      ELSIF NOT Expr.IsZeroes (field.dfault) THEN
        ...
        IR.Boost_addr_alignment (p.fieldAlign);
        IR.Add_offset (field.offset);
        fieldExprAlign := IR.GCD (p.fieldAlign, field.offset);
    ...
    IR.End_procedure (proc);
    RETURN proc;
  END GenInitProc;

PROCEDURE GenLinkProc (p: P;  defaults: INTEGER): IR.Proc =
  VAR
    v         := Scope.ToList (p.methods);
    method    : Method.Info;
    top       : Value.T;
  ...
  BEGIN
    ...
    WHILE (v # NIL) DO
      ...
      IF (method.dfault # NIL) THEN
        ...
        IR.Add_offset (method.offset);
        m_offset := MethodOffset (tVisible);
        IF (m_offset >= 0) THEN
          IR.Add_offset (m_offset);
        ELSE
