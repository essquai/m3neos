...
IMPORT IR, CallExpr, Expr, ExprRep, Procedure, Type, SubrangeType, Formal;
...
PROCEDURE Compile (ce: CallExpr.T) =
  BEGIN
    ...
    IR.Shift (Word_types[rep].cg_type);
  END Compile;
...
PROCEDURE CompileL (ce: CallExpr.T) =
  BEGIN
    ...
    IR.Shift_left (Word_types[rep].cg_type);
  END CompileL;
...
PROCEDURE CompileR (ce: CallExpr.T) =
  BEGIN
    ...
    CheckExpr.EmitChecks (ce.args[1], TInt.Zero, max,
                          IR.RuntimeError.ValueOutOfRange);
    IR.Shift_right (Word_types[rep].cg_type);
  END CompileR;
