...
IMPORT ObjectType, RefType, Scanner, Token, Module, ValueRep, IR;
...
PROCEDURE GenList (s: Set;  cnt: INTEGER;  eq: BOOLEAN): INTEGER =
  ...
  BEGIN
    ...
    FOR i := 0 TO seq.sz - 1 DO
      IR.Init_intt (offs + M3RT.RV_lhs_id, Target.Integer.size, array[i].lhs_id, TRUE);
      IR.Init_intt (offs + M3RT.RV_rhs_id, Target.Integer.size, array[i].rhs_id, TRUE);
...
PROCEDURE DeclareRevelation (t: T;  exported: BOOLEAN;
  ...
  BEGIN
    ...
    IF (exported) THEN
      IF (t.equal) THEN
        IR.Reveal_opaque (lhs_uid, rhs_uid);
