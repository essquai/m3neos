...
IMPORT M3, M3ID, IR, Value, ValueRep, Error, RunTyme;
...
REVEAL
  T = Value.T BRANDED "Variable.T" OBJECT
        ...
        mem_type    : BITS (*4*)8 FOR IR.Type := FIRST (IR.Type);
        stk_type    : BITS (*4*)8 FOR IR.Type := FIRST (IR.Type);
...
(* EXPORTED *)
PROCEDURE Reset () =
(* Toss as garbage, any IR.Var nodes that we've created *)
  VAR t, u: T;
  BEGIN
    t := TsWCGVars;
    WHILE (t # NIL) DO
...
PROCEDURE New (name: M3ID.T;  used: BOOLEAN): T =
  ...
  BEGIN
    ...
    t.readonly    := FALSE;
    t.extName     := M3ID.NoID;
    t.mem_type    := IR.Type.Void;
    t.stk_type    := IR.Type.Void;
    RETURN t;
  END New;
...
PROCEDURE LoadLValue (t: T) =
  ...
  BEGIN
    ...
    ELSE
      IR.Load_addr_of
        (t.cg_var, t.offset, IR.GCD (type_info.alignment, t.offset));
    END;
    IR.Boost_addr_alignment (t.cg_align);
  END LoadLValue;
...
PROCEDURE SetLValue (t: T) =
  ...
  BEGIN
    ...
    IF (v = NIL) THEN
      ...
      align := IR.Max_alignment;
    END;
    <*ASSERT t.indirect *>
    IR.Boost_addr_alignment (t.cg_align);
    IR.Store_addr (v, t.offset);
  END SetLValue;

(* EXPORTED *)
PROCEDURE LocalCGName (t: T;  VAR unit: IR.Var;  VAR offset: INTEGER) =
  BEGIN
    t.used := TRUE;
    Value.Declare (t);
    IF (t.initPending) THEN ForceInit (t); END;
...
PROCEDURE Declare (t: T): BOOLEAN =
  VAR
    ...
    mtype      := Type.IRType (t.type, in_memory := TRUE);
  ...
  BEGIN
    ...
    IF (is_struct) THEN mtype := IR.Type.Struct; END;

    IF (t.indirect) THEN
      Type.Typename (t.type, typename);
      typeUID := IR.Declare_indirect (typeUID, typename);
      typename := M3ID.NoID;
      size := Target.Address.size;
      align := Target.Address.align;
      mtype := IR.Type.Addr;
    ...
    IF (t.external) THEN
      ...
      END;
      t.cg_var := IR.Import_global (externM3ID, size, align, mtype, typeUID, typename);
      t.cg_align := align;
    ...
    ELSIF (t.global) THEN
      <*ASSERT t.allocated*>
      IR.Declare_global_field (t.name, t.offset, size, typeUID, FALSE);
      IF (t.initZero) THEN t.initDone := TRUE END;
      t.cg_align := align;
      IF (t.indirect) THEN
        t.cg_align := t.align;
        t.nextTWACGVar := TsWCGVars;  TsWCGVars := t;
        t.bss_var := IR.Declare_global (t.name, t.size, t.cg_align,
                              IR.Type.Struct, Type.GlobalUID (t.type),
                              exported := FALSE, init := FALSE);
        IR.Init_var (t.offset, t.bss_var, 0, FALSE);
      END;
    ...
    ELSIF (t.formal = NIL) THEN
      ...
      t.cg_align := align;
      t.nextTWACGVar := TsWCGVars;  TsWCGVars := t;
      t.cg_var := IR.Declare_local (t.name, size, align, mtype, typeUID,
                                    t.need_addr, t.up_level, IR.Maybe);
    ELSE
      (* parameter *)
      IF (t.indirect) THEN
      ...
      t.cg_align := t.align;
      t.nextTWACGVar := TsWCGVars;  TsWCGVars := t;
      t.cg_var := IR.Declare_param (t.name, size, align, mtype, typeUID, t.need_addr, t.up_level, IR.Maybe, typename);
    END;
...
PROCEDURE ConstInit (t: T) =
  ...
  BEGIN
    ...
    IF t.initStatic AND NOT t.initAllocated THEN
      ...
      IF (t.indirect) THEN
        typeUID  := IR.Declare_indirect (typeUID);
...
(* POST: TOS replaced by pointer to dope of copy. *) 
  ...
  BEGIN
    ...
    IR.Push(oldDopePtr);
    IR.Add_offset (M3RT.OA_sizes);
    IR.Store_addr (sizes, M3RT.OA_elt_ptr);
    IR.Load_intt (depth);
    IR.Store_int (Target.Integer.cg_type, sizes, M3RT.OA_size_0);
    ...
    IF Target.DefaultCall.args_left_to_right THEN
      ...
      IR.Load_addr_of (sizes, 0, Target.Address.align);
      IR.Pop_param (IR.Type.Addr);
    ELSE
      IR.Load_addr_of (sizes, 0, Target.Address.align);
      IR.Pop_param (IR.Type.Addr);
    ...
    IR.Open_elt_ptr (align); (* Addr of the new elements. *)
    IR.ForceStacked ();
    IR.Push(oldDopePtr);
    IR.Open_elt_ptr (align); (* Addr of the old elements. *)
    IR.ForceStacked ();
    ...
    IR.Free_temp (sizes);
    IR.Free (oldDopePtr);
    IR.Free (newDopePtr); (* It's now safely on the stack, so this is OK. *) 
  END CopyOpenArray;
      ...
      ELSIF t.initAllocated THEN
        ...
        IF Expr.CheckUseFailure (t.initExpr) THEN
          ...
          IF OpenArrayType.Is (initRepType) THEN
            ...
            IR.Open_elt_ptr (openEltAlign);
          END;
          IR.Copy (t.size, overlap := FALSE);
        ELSE
