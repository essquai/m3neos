...
IMPORT NamedType, TInt, WCharr, IR, Brand, Int, LInt;
...
PROCEDURE E3 (types: BOOLEAN;  ): Expr.T =
  ...
  BEGIN
    ...
    WHILE (cur.token IN RelOps) DO
      ...
      b := E4 (FALSE);
      CASE t OF
      | TK.tEQUAL   => a := EqualExpr.New (a, b, IR.Cmp.EQ);
      | TK.tSHARP   => a := EqualExpr.New (a, b, IR.Cmp.NE);
      | TK.tLESS    => a := CompareExpr.New (a, b, IR.Cmp.LT);
      | TK.tLSEQUAL => a := CompareExpr.New (a, b, IR.Cmp.LE);
      | TK.tGREATER => a := CompareExpr.New (a, b, IR.Cmp.GT);
      | TK.tGREQUAL => a := CompareExpr.New (a, b, IR.Cmp.GE);
