...
IMPORT M3, IR, Expr, ExprRep, Type, Int, LInt, Error, M3Buf, Target, TInt;
...
PROCEDURE Compile (p: P; StaticOnly: BOOLEAN) =
  BEGIN
    ...
    IR.Load_integer (Type.IRType (p.type), p.value);
  END Compile;
...
PROCEDURE GenLiteral (p: P;  offset: INTEGER;  type: Type.T;  is_const: BOOLEAN) =
  ...
  BEGIN
    ...
    IR.Init_int (offset, info.size, p.value, is_const);
  END GenLiteral;
