...
IMPORT M3, IR, Expr, ExprRep, M3String, Textt, Type, M3Buf;
...
VAR globalConstsCGVar : IR.Var  := NIL;
...
PROCEDURE BuildMethodList (): INTEGER =
  ...
  BEGIN
    ...
    FOR i := FIRST (Methods) TO LAST (Methods) DO
      IR.Init_proc (offs, Procedure.CGName (RunTyme.LookUpProc (i)), TRUE);
...
(* NOTE^ If ever duplicate text constants are removed as for arrays, this
    will need to change. *)
    IR.Load_addr_of (globalConstsCGVar, literals[uid] + Target.Address.pack,
...
(* Put a pointer, at 'offset' w/in the 'is_const' static area, to the
  ...
  VAR uid := SetUID (p);
  BEGIN
    IR.Init_var (offset, globalConstsCGVar, literals[uid] + Target.Address.pack,
                 is_const);
  END GenLiteral;
