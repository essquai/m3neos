...
IMPORT M3, IR, Expr, ExprRep, Type, ObjectType, M3Buf, Target;
...
PROCEDURE GenLiteral (p: P;  offset: INTEGER;  type: Type.T;  is_const: BOOLEAN) =
  ...
  VAR var    := IR.Import_global (M3ID.Add (name), Target.Address.size,
                                  Target.Address.align, IR.Type.Addr, uid);
  BEGIN
    IR.Init_var (offset, var, 0, is_const);
    Type.Compile (type);
    Error.ID (p.name,
