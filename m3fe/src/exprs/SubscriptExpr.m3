...
IMPORT IR, Expr, ExprRep, ArrayType, Error, Type, Int, LInt;
...
TYPE
  P = ExprRep.Tab BRANDED "SubscriptExpr.P" OBJECT
        ...
        tmp          : IR.Val;
        OMDopeVal    : IR.Val (* Dope of outermost array, if it's open. *);
  ...
  BEGIN
    IF TInt.LT (minb, mini) AND TInt.LT (maxi, maxb) THEN
      ...
      p.biased_b := CheckExpr.New (p.biased_b, TInt.Zero, z,
                                   IR.RuntimeError.SubscriptOutOfRange);
  ...
  BEGIN
    ...
    IF ArrayType.Split(ta, ti, te) THEN
      IF ti # NIL THEN (* Fixed array. *)
        ...
        END
      END;
      RETURN IR.GCD (arrayAlign, eltPack);
    END;
...
PROCEDURE CompileLV (p: P; traced := FALSE; StaticOnly: BOOLEAN) =
  ...
  BEGIN
    ...
    (* p.a has fixed array type.
       Top of IR stack is address of the elements.
       Replace it by address of subscripted element. *) 

      IF StaticSs (p, subscript) THEN
        IF subscript # 0 THEN
          IR.Add_offset (subscript * elt_pack (* bits. *));
        END 
      ELSE
        Expr.Compile (p.biased_b);
        IF Type.IsSubtype (tb, LInt.T) THEN
          IR.Loophole (Target.Longint.cg_type, Target.Integer.cg_type);
        END;
        ArrayType.GenIndex (ta);
        IR.Boost_addr_alignment (ArrayType.EltAlign (ta));
      END;

    ELSE (* p.a has open array type. *) 
      IF p.leftSs = NIL THEN
        (* This is the leftmost subscript, to the outermost array.  Top of IR stack
           is address of the dope.  Save the dope in p.OMDopeVal and replace it
           on the IR stack by the elements' addr. *) 
        p.OMDopeVal := IR.Pop ();
        IR.Push (p.OMDopeVal);
        IR.Open_elt_ptr (ArrayType.EltAlign (ta));
      ...
      ELSE (* Copy OMDopeVal and OMOpenDepth up from the subscript to the left. *) 
        ...
        (* Top of IR stack is already the address of the elements. *)
      ...
      (* This is the RM subscript, but p.a has additional open dimensions after
         ...
         dope and leave its address on the IR stack.  Check will have prevented
        ...
        IF Host.doRangeChk THEN (* range check the subscript *)
          IR.Push (p.OMDopeVal);
          IR.Open_size (p.shapeSs);
          IR.Check_index (IR.RuntimeError.SubscriptOutOfRange);
        END;

        FOR i := 0 TO newShapeLast DO
          IR.Load_int (Target.Integer.cg_type,
                       newDopeVar, M3RT.OA_sizes + i * Target.Integer.pack);
          IR.Multiply (Target.Word.cg_type);
        END;
        ...
        IR.Store_addr (newDopeVar, M3RT.OA_elt_ptr);
        IR.Load_addr_of_temp (newDopeVar, 0, Target.Address.align);
      ELSE
      (* Either there is another subscript to the right, or the element
         is not an array.  Either way, leave the address of the element
         on top of the IR stack.  It will be a IR.Val, and can represent
        ...
        FOR I := p.shapeSs + 1 TO p.OMOpenDepth - 1 DO 
          ...
          IR.Multiply (Target.Word.cg_type); 
        END;
(* CHECK: Can we just use some IR op directly here? See IR.Index_bits. *)
        ArrayType.GenIndex (ta); 
        IR.Boost_addr_alignment (OpenArrayType.EltAlign (ta));
      END;
    END;
  END CompileLV;
