...
IMPORT M3, M3ID, IR, Expr, ExprRep, Value, Type, Module;
...
PROCEDURE QualifyExprAlign (p: P): Type.BitAlignT =
  ...
  BEGIN
    ...
    CASE p.class <*NOWARN*>
    OF Class.recField =>
        result := IR.GCD (Expr.Alignment (p.lhsExpr), fieldInfo.offset);
    ...
    | Class.objField =>
        ...
        END;
        result := IR.GCD (MAX (fieldsAlign, fieldTypeAlign), fieldInfo.offset);
        <*ASSERT result MOD fieldTypeAlign = 0 *>
        RETURN result;
    END (*CASE*)
...
PROCEDURE Compile (p: P; StaticOnly: BOOLEAN) =
  ...
  BEGIN
    ...
    | Class.recField =>
        ...
        END;
        IR.Add_offset (fieldInfo.offset);
