...
IMPORT IR, Expr, ExprRep, Error, ProcType, Type, UserProc;
...
PROCEDURE PrepBR (p: T;  true, false: IR.Label;  freq: IR.Frequency) =
  BEGIN
    p.methods.prepBR (p, true, false, freq);
  END PrepBR;

PROCEDURE CompileBR (p: T;  true, false: IR.Label;  freq: IR.Frequency) =
  BEGIN
