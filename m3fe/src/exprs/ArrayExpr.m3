...
(* An array constructor can use one of these protocols for getting
   ...
   assign protocol:     The value is built directly into a runtime
                        ...
                        IR stack) during Prep.
*)

IMPORT Fmt;

IMPORT M3, M3ID, IR, Expr, ExprRep, Error, Type, ArrayType, PackedType;
...
TYPE LevelInfoTyp = RECORD
    ...
    fixingLenVal      : IR.Val := NIL;
                        (* If this is a dynamic level, its runtime length. *)
    eltBytepackVal    : IR.Val := NIL;
...
TYPE ResultKindTyp
  = { RKUnknown      (* Initial value. *)
    ...
    , RKTempElts     (* Static-sized IR.Var temp, elements only. *)
                       (* Uses top.buildTempVar. *)
    , RKTempStatic   (* Static-sized IR.Var temp, with dope. *)
                       (* Uses top.buildTempVar. *)
...
(* CLEANUP ^ tipe always duplicates field "type", inherited from Expr.T *) 
    ...
    finalVal          : IR.Val;
    buildAddrVal      : IR.Val;
    buildEltsAddrVal  : IR.Val;
    buildTempVar      : IR.Var;
    dynByteOffsetVal  : IR.Val := NIL; (* Top only. *)
    shallowestDynDepth: INTEGER := FIRST (INTEGER);
    deepestDynDepth   : INTEGER := FIRST (INTEGER);
    heapTempPtrVar    : IR.Var := NIL;
    ...
    firstArgDopeVal   : IR.Val;
    lastArgExpr       : Expr.T := NIL;
    levels            : LevelsTyp;
    RTErrorMsg        : TEXT := NIL;
    RTErrorCode       := IR.RuntimeError.Unknown;
...
(* REVIEW: Initializations here vs. declared in T. *)
    ...
    constr.RTErrorCode          := IR.RuntimeError.Unknown;
...
PROCEDURE MergeRTError (top: T; Code: IR.RuntimeError; Msg: TEXT) =
  BEGIN
...
PROCEDURE CheckRecurse
  ...
  VAR argConstr: T;
  VAR key: M3ID.T;
  VAR RTErrorCode: IR.RuntimeError;
  VAR RTErrorMsg: TEXT;
            ...
            ELSIF locStaticLen # levelInfo.staticLen THEN
              ...
              Error.Warn
                (depth,
                 ...
                 & Fmt.Int (depth));
              MergeRTError
                (top, IR.RuntimeError.IncompatibleArrayShape,
...
(* Arrange to use LHS from the IR stack to set nonstatic shape components. *)
(* Will look through a ConsExpr. *)
  VAR top: T;
  BEGIN
    top := ArrayConstrExpr (expr);
...
PROCEDURE GenEvalFixingInfo
  (top: T; fixingDopeVal: IR.Val; fixingExprDepth: INTEGER) =
  ...
  BEGIN
    ...
    LOOP
      ...
      WITH levelInfo = top.levels^[depthWInTopConstr] DO
        IR.Push (fixingDopeVal);
        IR.Open_size (depthWInFixingExpr) (* *length* *);
        levelInfo.fixingLenVal := IR.Pop ();
        IR.Push (levelInfo.fixingLenVal);
        IF depthWInTopConstr < top.deepestDynDepth OR eltByteSize > 1 THEN
          IR.Multiply (Target.Integer.cg_type);
        END; 
        levelInfo.eltBytepackVal := IR.Pop ();
        IF depthWInTopConstr <= top.shallowestDynDepth THEN
          EXIT
        ELSE 
          IR.Push (levelInfo.eltBytepackVal);
...
PROCEDURE InitVarDope (top: T; var: IR.Var) =
(* PRE: top.depth = 0. *)
  VAR length: INTEGER;
  BEGIN
    IF top.repOpenDepth > 0 THEN
...
PROCEDURE InitValDope (top: T; val: IR.Val; VAR eltsAddrVal: IR.Val) =
(* PRE: top.depth = 0. *)
  VAR length: INTEGER;
  BEGIN
    IF top.repOpenDepth <= 0 THEN
    ...
    ELSE
      ...
      IR.Push (val);
      IR.Boost_addr_alignment (Target.Address.align);
      IR.Add_offset (top.dopeSize);
      eltsAddrVal := IR.Pop ();
      IR.Push (eltsAddrVal);
      IR.Boost_addr_alignment (top.topEltsAlign);
      IR.Store_indirect
...
(* PRE: top.depth = 0. *)
  (* Must be called IFF top is the top of a tree of array constructors. *)
  (* PRE LHS addr is on IR stack IFF UsesAssignProtocol. *)
  ...
  BEGIN
    ...
    | RKTyp.RKDirectElts => (* LHS elements address is atop the IR stack. *)
      IR.Boost_addr_alignment (top.topRepAlign);
      top.buildEltsAddrVal := IR.Pop (); (* LHS points to elements. *)

    | RKTyp.RKDirectDoped => (* LHS dope address is atop the IR stack. *)
      IR.Boost_addr_alignment (Target.Address.align);
      top.buildAddrVal := IR.Pop (); (* LHS points to dope. *)
...
(* TODO: IR.Declare_whatever, for this and other declared temps. *)
      ...
      IF UseTargetVar (top) THEN (* LHS address is on the IR stack. *)
        IR.Boost_addr_alignment (top.topRepAlign);
        top.finalVal := IR.Pop ();
      END;
    ...
    | RKTyp.RKTempDyn =>
      ...
      IR.Store_addr (shapeVar, M3RT.OA_elt_ptr); (* Elements pointer. *)

      IR.Load_intt (top.repOpenDepth);
      IR.Store_int
      ...
      FOR depth := 0 TO top.shallowestDynDepth - 1 DO
        ...
        <* ASSERT length >= 0 *>
        IR.Load_intt (length);
        IR.Store (shapeVar, offset, Target.Integer.size, Target.Integer.align
      ...
      FOR depth := top.shallowestDynDepth TO top.deepestDynDepth DO
        IR.Push (top.levels^ [depth].fixingLenVal);
        IR.Store (shapeVar, offset, Target.Integer.size, Target.Integer.align
                  , Target.Integer.cg_type);
        INC (offset, Target.Integer.size);
      END;
...
(* TODO: Would it be possible & better to allocate this untraced? *)        
      ...
      IF Target.DefaultCall.args_left_to_right THEN
        ...
        IR.Load_addr_of (shapeVar, 0, Target.Address.align);
        IR.Pop_param (IR.Type.Addr);
      ELSE
        IR.Load_addr_of (shapeVar, 0, Target.Address.align);
        IR.Pop_param (IR.Type.Addr);
      ...
      IR.Push (top.buildAddrVal);
      IR.Boost_addr_alignment (Target.Address.align);
      IR.Open_elt_ptr (top.topEltsAlign);
      top.buildEltsAddrVal:= IR.Pop ();
      ...
      top.heapTempPtrVar
        := IR.Declare_local
             (name, Target.Address.size, Target.Address.align, IR.Type.Addr,
              Type.GlobalUID (top.refType), in_memory := TRUE, up_level := FALSE,
              f := IR.Maybe);
      IR.Push (top.buildAddrVal);
      IR.Boost_addr_alignment (Target.Address.align);
      IR.Store_addr (top.heapTempPtrVar, (*Offset:=*) 0);
    ...
    (* Free temporaries: *)
    IR.Free_temp (shapeVar);
    IF top.shallowestDynDepth >= 0 THEN
      FOR depth := top.shallowestDynDepth TO top.deepestDynDepth DO
        IR.Free (top.levels^ [depth].fixingLenVal);
        IR.Free (top.levels^ [depth].eltBytepackVal);
      END;
    END;
    IR.Free (top.dynByteOffsetVal);
(* CHECK: Will Free_temps get done sometime? *)
    top.state := StateTyp.Prepped;
  END InnerPrep;

PROCEDURE GenCopyOpenArgValueDyn
  (top, constr: T; argDopeAddrVal: IR.Val; eltAlign: Type.BitAlignT) =
(* PRE: TOS is a IR.Val for address (w/in LHS) to copy to. *)
(* PRE: top.shallowestDynDepth >= 0 *)
  ...
  BEGIN
    ...
    IR.ForceStacked ();
    (* Source address: *)
    IR.Push (argDopeAddrVal);
    IR.Boost_addr_alignment (Target.Address.align);
    IR.Open_elt_ptr (eltAlign);
    IR.ForceStacked ();
    (* Size in bytes: *)
    <* ASSERT top.shallowestDynDepth >= 0 *>
    <* ASSERT constr.depth < top.shallowestDynDepth *>
    IR.Push (top.levels^[top.shallowestDynDepth].eltBytepackVal);
    staticFlatEltCt
      := top.levels^[constr.depth + 1(*Of Arg.*)].staticFlatEltCt;
    IF staticFlatEltCt # 1 THEN
      IR.Load_intt (staticFlatEltCt);
      IR.Multiply (Target.Integer.cg_type);
    END;
(* TODO: call NoteWrite, as needed. *)

    IR.Copy_n (Target.Byte, overlap := FALSE);
...
(* Push the address of the element at bitOffset within the elements
  ...
  BEGIN
    ...
    => IR.Push (top.buildEltsAddrVal);
      IF bitOffset # 0 THEN
        <* ASSERT bitOffset MOD eltAlign = 0 *>
        IR.Add_offset (bitOffset);
        IR.Boost_addr_alignment (eltAlign);
      END;
    | RKTyp.RKTempElts
    => IR.Load_addr_of (top.buildTempVar, bitOffset, eltAlign);
    | RKTyp.RKTempStatic
    => IR.Load_addr_of
...
PROCEDURE PushDynBytesizeVal (top: T; depth: INTEGER) =
  ...
  BEGIN
    ...
    IF depth > top.deepestDynDepth THEN
      ...
      <* ASSERT eltPack >= 0 *>
      <* ASSERT eltPack MOD Target.Byte = 0 *>
      IR.Load_intt (eltPack DIV Target.Byte);
    ELSIF depth < top.shallowestDynDepth THEN
      ...
      END
    ELSE
      IR.Push (top.levels^[depth].eltBytepackVal);
    END;
...
(* selfFlatOffset is the flattened offset from the beginning of top to the
   beginning of constr. *) 
  VAR argRepType: Type.T;
  VAR dotSsVal, argAddrVal: IR.Val := NIL;
  VAR topLab: IR.Label;
  VAR argCt: INTEGER;
        ...
        WITH wArgExpr = constr.args^[i] DO
          ...
          IF wArgExpr # NIL THEN
            ...
            (* Not a constructor.  Handle wArgExpr here. *)
              ...
              eltAlign
                := IR.GCD (top.topEltsAlign, argFlatOffset MOD Target.Word.size);
              depthWInArg := 0;
...
(* TODO: Make do_direct work transitively through here. *)
                ...
                THEN (* argAddrVal is address of arg's dope. *)
                  ...
                  LOOP
                    ...
                    WITH openLevelInfo = top.levels^ [depthWInTopConstr] DO
                      ...
                      IR.Push (argAddrVal);
                      IR.Boost_addr_alignment (Target.Integer.align);
                      IR.Open_size (depthWInArg);
                      <* ASSERT openLevelInfo.staticLen >= 0 *>
                      IR.Load_intt (openLevelInfo.staticLen);
                      IR.Check_eq
                        (Target.Integer.cg_type, IR.RuntimeError.IncompatibleArrayShape);
                ...
                IR.Copy (constrEltPack, overlap := FALSE);
              ELSE
              ...
              (* The dynamic case.  top.shallowestDynDepth >= 0.
                ...
                LOOP (* Through open levels of arg. *)
                  ...
                  WITH openLevelInfo = top.levels^ [depthWInTopConstr] DO
                    ...
                    THEN (* Arg is open but statically constrained at this level. *)
                      ...
                      IR.Check_eq
                        (Target.Integer.cg_type,
                         IR.RuntimeError.IncompatibleArrayShape);
                    ELSE
                      top.useTargetVarLocked := TRUE;
                      IF UseTargetVar (top)
                      ...
                      THEN (* Arg is open and dynamically constrained at this level. *)
                        ...
                        IR.Push (openLevelInfo.fixingLenVal);
                        IR.Check_eq
                          (Target.Integer.cg_type,
                           IR.RuntimeError.IncompatibleArrayShape);
                      END;
                ...
                THEN (* First time. Dynamic offset is zero. *)
                  ...
                  top.dynByteOffsetVal := IR.Pop ();
                ELSE
                  IR.ForceStacked ();
                  IR.Push (top.dynByteOffsetVal);
  ...
  (* TODO: Is it worth it to unroll this loop for low iteration counts, or
	    ...
	    IR.Load_intt (argCt);
	    dotSsVal := IR.Pop_temp ();
	    topLab := IR.Next_label ();
	    IR.Set_label (topLab);
  ...
  (* CHECK: That this won't straddle word boundary. *)
	    ...
	    IR.Load_intt (1);
	    IR.Add (Target.Integer.cg_type);
	    IR.Store_temp (dotSsVal);

	    (* Gen RT code for IF (dotSsVal < NUMBER(ARRAY) GOTO topLab *)
	    IR.Push (dotSsVal);
	    IR.Load_intt (constr.eltCt);
	    IR.If_compare (Target.Integer.cg_type, IR.Cmp.LT, topLab, IR.Likely);
...
(* Initialize dope fields of literal. *)
  ...
  BEGIN
    ...
    FOR i := 0 TO top.repOpenDepth - 1 DO
      ...
      IR.Init_intt (shapeOffset, Target.Integer.size, eltCt, inConstArea);
...
(* PRE: top.depth = 0 *)
  BEGIN
    ...
    => <* ASSERT NOT StaticOnly *>
      IR.Push (top.buildEltsAddrVal);
      IR.Free (top.buildEltsAddrVal);

    | RKTyp.RKDirectDoped
    => <* ASSERT NOT StaticOnly *>
      IR.Push (top.buildAddrVal);
      IR.Free (top.buildAddrVal);
    ...
    => (* We built into a static-sized temp Var. *) 
      ...
      IF top.finalVal = NIL THEN
        IR.Load_addr_of_temp (top.buildTempVar, 0, top.topRepAlign)
      ELSE
        IF top.state < StateTyp.Compiled THEN (* Don't copy twice. *)
          IR.Push (top.finalVal);
          IR.Load_addr_of_temp (top.buildTempVar, 0, top.topRepAlign);
          IR.Copy (top.totalSize, overlap := FALSE);
        END;
        IR.Push (top.finalVal);
        IR.Free (top.finalVal);
      END;
    ...
    => (* We built into a heap-allocated, dynamic-sized, contiguous
      ...
      <* ASSERT (top.finalVal = NIL) = (NOT UsesAssignProtocol (top)) *>
      IF top.finalVal = NIL THEN
        IR.Push (top.buildAddrVal) (* Return the temp. *)
...
(* CHECK^ Actually, do we want to copy multiple times? *)        
          IR.Push (top.finalVal);
          IR.Push (top.buildAddrVal);
          IR.Load_intt (top.levels^[0].staticFlatEltCt);
(* TODO ^Eliminate multiply by one. *)
          <* ASSERT top.shallowestDynDepth >= 0 *>
          IR.Push (top.levels^[top.shallowestDynDepth].eltBytepackVal);
          IR.Multiply (Target.Integer.cg_type);
          IR.Load_intt (top.dopeSize);
          IR.Add (Target.Integer.cg_type);
          IR.Copy_n (s := 1(*Unit size*), overlap := FALSE);
        END;
        IR.Push (top.finalVal);
        IR.Free (top.finalVal);
...
PROCEDURE CheckStaticRTErrEval
  (expr: Expr.T; VAR(*OUT*) Code: IR.RuntimeError; VAR(*OUT*) Msg: TEXT) =
...
(* Return the first-discovered error found and stored during Check. *)
  VAR constrExpr: T;
  BEGIN
    IF Code # IR.RuntimeError.Unknown THEN RETURN END;
    constrExpr := ArrayConstrExpr (expr);
    TYPECASE constrExpr OF
    | NULL =>
    | T(top) =>
      <* ASSERT top.state >= StateTyp.Checked *>
      IF top.RTErrorCode # IR.RuntimeError.Unknown THEN
...
PROCEDURE CheckStaticRTErrAssign
  (lhsType: Type.T; expr: Expr.T;
   VAR(*OUT*) Code: IR.RuntimeError; VAR(*OUT*) Msg: TEXT) =
        ...
        WITH levelInfo = top.levels^[depth] DO
          ...
          IF indexType # NIL AND levelInfo.staticLen >= 0 THEN
            ...
            THEN (* Found a static runtime shape mismatch. *)
              ...
              IF Code = IR.RuntimeError.Unknown THEN (* It's the first. *)
                Code := IR.RuntimeError.IncompatibleArrayShape;
                Msg := locMsg;
              END
            END
          END
