...
IMPORT IR, CallExpr, Expr, ExprRep, Procedure, Target, TInt, M3ID;
...
PROCEDURE Check (ce: CallExpr.T;  VAR cs: Expr.CheckState) =
  ...
  BEGIN
    ...
    IF EnumExpr.Split (ce.args[4], order, t) AND TInt.ToInt (order, failure) THEN
      ...
      END;
    ELSE
      failure := ORD(IR.MemoryOrder.Sequential);
...
PROCEDURE Compile (ce: CallExpr.T) =
  ...
  BEGIN
    ...
    IR.Compare_exchange (Type.IRType(Rep.T, in_memory := TRUE),
                         success := VAL(success, IR.MemoryOrder),
                         failure := VAL(failure, IR.MemoryOrder));
