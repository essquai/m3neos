...
IMPORT IR, CallExpr, Expr, Type, Procedure, First, ArrayType;
  ...
  BEGIN
    ...
    ELSIF Type.GetBounds (t, min, max) THEN (* ordinal type *)
      IF Type.IsSubtype (t, LInt.T)
        THEN IR.Load_integer (Target.Longint.cg_type, max);
        ELSE IR.Load_integer (Target.Integer.cg_type, max);
      END;
    ELSIF Type.IsEqual (t, Reel.T, NIL) THEN
      IR.Load_float (Target.Real.max);
    ELSIF Type.IsEqual (t, LReel.T, NIL) THEN
      IR.Load_float (Target.Longreal.max);
    ELSIF Type.IsEqual (t, EReel.T, NIL) THEN
      IR.Load_float (Target.Extended.max);
    ELSE
      <* ASSERT FALSE *>
    END;
  END Compile;
