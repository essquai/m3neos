...
IMPORT IR, Expr, Token, Scanner, Stmt, StmtRep, Error;
  ...
  BEGIN
    IF Host.doAsserts THEN
      ...
      IF (v = NIL) THEN
        ...
        Expr.PrepBranch (p.cond, skip, IR.No_label, IR.Always);
        Expr.CompileBranch (p.cond, skip, IR.No_label, IR.Always);
        Crash (p);
        IR.Set_label (skip);
...
PROCEDURE Crash (p: P) =
  ...
  BEGIN
    IF (p.msg = NIL) THEN
      IR.Abort (IR.RuntimeError.AssertFailed);
      RETURN;
    END;
