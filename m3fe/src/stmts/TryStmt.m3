...
IMPORT M3ID, IR, Variable, Scope, Exceptionz, Value, Error, Marker;
...
TYPE
  Except = REF RECORD
           ...
           END;
TYPE
  Caught = REF ARRAY OF IR.TypeUID;
...
PROCEDURE Check (p: P;  VAR cs: Stmt.CheckState) =
  ...
  BEGIN
    ...
    IF (p.hasElse) THEN
      Marker.PushTryElse (IR.No_label, IR.No_label, IR.No_label, NIL);
      handled := ESet.NewAny ();
    ELSE
      ...
      p.handled := handled;
      Marker.PushTry (IR.No_label, IR.No_label, IR.No_label, NIL, handled);
    END;
...
PROCEDURE Compile1 (p: P): Stmt.Outcomes =
  VAR
    oc: Stmt.Outcomes;
    l, next_handler, handler_body: IR.Label;
    info, next_info: IR.Var;
    h: Handler;
    another: BOOLEAN;
    catches : Caught;
  BEGIN
    ...
    info := IR.Declare_local (M3ID.NoID, Target.Address.size,
    ...
    IR.Gen_location (p.h_origin);

    (* generate the landing pad *)
    catches := Handled(p);
    IR.Landing_pad (l+1, catches^);
    IR.Store_addr (info);
...
PROCEDURE CompileHandler1 (h: Handler;  info: IR.Var;
                         resume: IR.Label;  last: BOOLEAN): Stmt.Outcomes =
  VAR
    e: Except;
    oc: Stmt.Outcomes;
    zz: Scope.T;
  ...
  BEGIN
    ...
    IF (NOT last) THEN
      ...
      WHILE (e # NIL) DO
        ...
        e := e.next;
        IF (e # NIL) THEN
          IR.If_compare (Target.Integer.cg_type, IR.Cmp.EQ, top, IR.Maybe);
          need_top := TRUE;
        ELSE
          IR.If_compare (Target.Integer.cg_type, IR.Cmp.NE, top+1, IR.Maybe);
        END;
      END;
    END;

    IF (need_top) THEN IR.Set_label (top) END;
    ...
    IF (h.scope # NIL) THEN
      zz := Scope.Push (h.scope);
        ...
        IF (Stmt.Outcome.FallThrough IN oc) THEN IR.Jump (resume) END;
...
PROCEDURE Compile2 (p: P): Stmt.Outcomes =
  ...
  BEGIN
    ...
    IR.Load_nil ();
    IR.Store_addr (frame, M3RT.EF1_exception);
    ***********************************************)
    ...
    ELSE
      ...
      IR.Load_addr_of (eset, eoffset, Target.Integer.align);
      IR.Store_addr (frame, M3RT.EF1_handles);
...
PROCEDURE CompileHandler2 (h: Handler;  frame: IR.Var;
                         resume: IR.Label;  last: BOOLEAN): Stmt.Outcomes =
  VAR
    e: Except;
    oc: Stmt.Outcomes;
  ...
  BEGIN
    ...
    IF (NOT last) THEN
      ...
      WHILE (e # NIL) DO
        ...
        e := e.next;
        IF (e # NIL)
          THEN IR.If_compare (Target.Integer.cg_type, IR.Cmp.EQ, top, IR.Maybe);  need_top := TRUE;
          ELSE IR.If_compare (Target.Integer.cg_type, IR.Cmp.NE, top+1, IR.Maybe);
        END;
      END;
      (*** IR.Jump (top+1); ***)
    END;
...
PROCEDURE PushHandler (info: IR.Var;  offset: INTEGER;   direct: BOOLEAN) =
  BEGIN
    WITH z = stack [tos] DO
      z.info   := info;
      z.offset := offset;
...
PROCEDURE LoadInfoPtr () =
  BEGIN
    IF (tos <= 0) THEN  IR.Load_nil ();  RETURN;  END;
