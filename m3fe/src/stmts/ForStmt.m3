...
IMPORT M3ID, IR, Error, Scope, Expr, Stmt, StmtRep;
...
PROCEDURE Compile (p: P): Stmt.Outcomes =
  VAR
    ...
    zz: Scope.T;
    index, to, by: IR.Var;
    t_index, t_to, t_by: IR.Val;
    l_top, l_test, l_less, l_exit: IR.Label;
    type: Type.T;
  ...
  BEGIN
    ...
    zz := Scope.Push (p.scope);
      ...
      ELSE
        ...
        index_copy := TRUE;
        index := IR.Declare_local (M3ID.NoID, TargetMap.CG_Size[cg_type],
      ...
      IF (from = NIL) THEN
        IR.Push (t_index);
        IR.Store_int (cg_type, index);
        IR.Free (t_index);
      ELSE
        IR.Load_integer (cg_type, from_val);
        IR.Store_int (cg_type, index);
      END;

      IF (limit = NIL) THEN
        (* declare the local variable *)
        to := IR.Declare_local (M3ID.NoID, TargetMap.CG_Size[cg_type],
                                TargetMap.CG_Align[cg_type], cg_type, uid,
                                in_memory := FALSE, up_level := FALSE,
                                f := IR.Maybe);
        IR.Push (t_to);
        IR.Store_int (cg_type, to);
        IR.Free (t_to);
      END;

      IF (step = NIL) THEN
        (* declare the local variable *)
        by := IR.Declare_local (M3ID.NoID, TargetMap.CG_Size[cg_type],
                                TargetMap.CG_Align[cg_type], cg_type, uid,
                                in_memory := FALSE, up_level := FALSE,
                                f := IR.Maybe);
        IR.Push (t_by);
        IR.Store_int (cg_type, by);
        IR.Free (t_by);
      END;
      ...
      IR.Load_int (cg_type, index);
      IF (step # NIL)
        THEN IR.Load_integer (cg_type, step_val);
        ELSE IR.Load_int (cg_type, by);
      END;
      IR.Add (cg_type);
      IR.Store_int (cg_type, index);
      ...
      IF (step # NIL) THEN (* constant step value *)
        IR.Load_int (cg_type, index);
        IF (limit # NIL)
          THEN IR.Load_integer (cg_type, limit_val);
          ELSE IR.Load_int (cg_type, to);
        END;
        IF TInt.LE (TInt.Zero, step_val)
          THEN IR.If_compare (cg_type, IR.Cmp.LE, l_top, IR.Likely);
          ELSE IR.If_compare (cg_type, IR.Cmp.GE, l_top, IR.Likely);
        END;
      ELSIF TInt.LE (TInt.Zero, step_min) THEN
        ...
        IF (limit # NIL)
          ...
          ELSE IR.Load_int (cg_type, to);
        END;
        IR.If_compare (cg_type, IR.Cmp.LE, l_top, IR.Likely);
      ELSIF TInt.LT (step_max, TInt.Zero) THEN
        ...
        IF (limit # NIL)
          ...
          ELSE IR.Load_int (cg_type, to);
        END;
        IR.If_compare (cg_type, IR.Cmp.GE, l_top, IR.Likely);
      ELSE (* variable step value *)
        l_less := IR.Next_label (2);
        IR.Load_int (cg_type, by);
        IR.Load_integer (cg_type, TInt.Zero);
        IR.If_compare (cg_type, IR.Cmp.LT, l_less, IR.Likely);
        IR.Load_int (cg_type, index);
        IF (limit # NIL)
          THEN IR.Load_integer (cg_type, limit_val);
          ELSE IR.Load_int (cg_type, to);
