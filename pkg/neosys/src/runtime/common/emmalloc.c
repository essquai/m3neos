...
// #define EMMALLOC_NO_STD_EXPORTS
...
// #define __EMSCRIPTEN_SHARED_MEMORY__
// In multithreaded builds, use a simple global spinlock strategy to acquire/release access to the memory allocator.
static emm_lock_t multithreadingLock = { ATOMIC_FLAG_INIT, 0};
#define MALLOC_ACQUIRE() emm_lock_grab(&multithreadingLock)
#define MALLOC_RELEASE() emm_lock_drop(&multithreadingLock)
// Test code to ensure we have tight malloc acquire/release guards in place.
#define ASSERT_MALLOC_IS_ACQUIRED() emm_lock_assert(&multithreadingLock)
...
static void dump_memory_regions() {
  ...
  for (int i = 0; i < NUM_FREE_BUCKETS; ++i) {
    ...
    while (fr != &freeRegionBuckets[i]) {
      emm_diag_stow("In bucket %d, free region %p, size: %zu (size at ceiling: %zu), prev: %p, next: %p)",
  ...
  emm_diag_stow("Free bucket index map: %d %d", (uint32_t)(freeRegionBucketsUsed >> 32), (uint32_t)freeRegionBucketsUsed);
  emm_diag_stow(" ");
}
...
static int validate_memory_regions() {
  ...
  while (root) {
    ...
    if (!debug_region_is_consistent(r)) {
      emm_diag_stow("Used region %p, size: %zu used/--FREE-- =%d is corrupt (size markers in the beginning and at the end of the region do not match!)",
        r, r->size, region_ceiling_size(r) == r->size);
      return 1;
    }
    uint8_t *lastRegionEnd = root->endPtr;
  ...
  for (int i = 0; i < NUM_FREE_BUCKETS; ++i) {
    ...
    while (fr != &freeRegionBuckets[i]) {
      if (!debug_region_is_consistent(fr) || !region_is_free(fr) || fr->prev != prev || fr->next == fr || fr->prev == fr) {
        emm_diag_stow("In bucket %d, free region %p, size: %zu (size at ceiling: %zu), prev: %p, next: %p is corrupt!",
          i, fr, fr->size, size_of_region_from_ceiling(fr), fr->prev, fr->next);
        return 1;
      }
      prev = fr;
...
static bool claim_more_memory(size_t numBytes) {
  ...
  emm_diag_stow("claim_more_memory(numBytes=%zu)", numBytes);
#endif
  ...
  uint8_t *startPtr = (uint8_t*)emm_sbrk_vary((int64_t)numBytes);
  if ((intptr_t)startPtr == -1) {
#ifdef EMMALLOC_VERBOSE
    emm_diag_stow("claim_more_memory: sbrk failed!");
#endif
    return false;
  }
#ifdef EMMALLOC_VERBOSE
  emm_diag_stow("claim_more_memory: claimed %p - %p (%zu bytes) via sbrk()", startPtr, startPtr + numBytes, numBytes);
#endif
...
#ifdef EMMALLOC_VERBOSE
  emm_diag_stow("claim_more_memory: created a sentinel memory region at address %p", endSentinelRegion);
#endif
...
#ifdef EMMALLOC_VERBOSE
    emm_diag_stow("claim_more_memory: sbrk() returned a region contiguous to last root region, expanding the existing root region");
#endif
...
#ifdef EMMALLOC_VERBOSE
    emm_diag_stow("claim_more_memory: sbrk() returned a disjoint region to last root region, some external code must have sbrk()ed outside emmalloc(). Creating a new root region");
#endif
...
#ifdef EMMALLOC_VERBOSE
  emm_diag_stow("initialize_emmalloc_heap()");
#endif
...
#ifdef EMMALLOC_VERBOSE
  emm_diag_stow("attempt_allocate(freeRegion=%p,alignment=%zu, size=%zu", freeRegion, alignment, size);
#endif
...
#ifdef EMMALLOC_VERBOSE
  emm_diag_stow("attempt_allocate - succeeded allocating memory, region ptr=%p, align=%zu, payload size=%zu bytes)", freeRegion, alignment, size);
#endif
...
#ifdef EMMALLOC_VERBOSE
  emm_diag_stow("allocate_memory(align=%zu, size=%zu bytes)", alignment, size);
#endif
...
#ifdef EMMALLOC_VERBOSE
    emm_diag_stow("Allocation failed: alignment not power of 2!");
#endif
    return 0;
  }
...
#ifdef EMMALLOC_VERBOSE
    emm_diag_stow("Allocation failed: attempted allocation size is too large: %zu bytes! (negative integer wraparound?)", size);
#endif
    return 0;
  }
...
#ifdef EMMALLOC_VERBOSE
  emm_diag_stow("Could not find a free memory block!");
#endif
...
void emmalloc_free(void *ptr) {
  ...
  if (size < sizeof(Region) || !region_is_in_use(region)) {
    if (debug_region_is_consistent(region)) {
      // LLVM wasm backend bug: cannot use emm_diag_stow() here, that generates internal compiler error
      // Reproducible by running e.g. other.test_alloc_3GB
      emm_diag_stow("Double free at region ptr %p, region->size: %p, region->sizeAtCeiling: %p", region, size, region_ceiling_size(region));
    } else {
      emm_diag_stow("Corrupt region at region ptr %p region->size: %p, region->sizeAtCeiling: %p", region, size, region_ceiling_size(region));
    }
  }
#endif
  assert(size >= sizeof(Region));
...
#ifdef EMMALLOC_VERBOSE
  emm_diag_stow("attempt_region_resize(region=%p, size=%zu bytes)", region, size);
#endif
...
#ifdef EMMALLOC_VERBOSE
  emm_diag_stow("attempt_region_resize failed.");
#endif
  return 0;
}
...
#ifdef EMMALLOC_VERBOSE
  emm_diag_stow("aligned_realloc(ptr=%p, alignment=%zu, size=%zu)", ptr, alignment, size);
#endif
...
struct mallinfo emmalloc_mallinfo() {
  ...
  emm_sbrk_stat(&max, &cur);
  info.arena = max - cur;
  // info.arena = emscripten_get_heap_size() - (size_t)emm_sbrk_vary(0);
...
static int trim_dynamic_heap_reservation(size_t pad) {
  ...
  if (!listOfAllRegions) {
#ifdef EMMALLOC_VERBOSE
    emm_diag_stow("emmalloc_trim(): cannot trim memory, emmalloc is currently not initialized to manage any dynamic memory at all.");
#endif
  ...
  void *sbrkAddr = emm_sbrk_vary(0);
#ifdef EMMALLOC_VERBOSE
    emm_diag_stow("emmalloc_trim(): emm_sbrk_vary(0) = %p, previousSbrkEndAddress = %p", sbrkAddr, previousSbrkEndAddress);
#endif
...
#ifdef EMMALLOC_VERBOSE
    emm_diag_stow("emmalloc_trim(): Last actual region %p is in use, there is nothing to trim from.", lastActualRegion);
#endif
    return 0;
  }
...
#ifdef EMMALLOC_VERBOSE
    emm_diag_stow("emmalloc_trim(): Last actual region does not have enough space to leave %zu bytes of free memory in it.", pad);
#endif
    return 0;
  }
...
#ifdef EMMALLOC_VERBOSE
    emm_diag_stow("emmalloc_trim(): shrinkAmount %zu.", shrinkAmount);
#endif
  shrinkAmount = (size_t)ALIGN_DOWN(shrinkAmount, __alignof__(max_align_t));
#ifdef EMMALLOC_VERBOSE
    emm_diag_stow("emmalloc_trim(): shrinkAmount2 %zu.", shrinkAmount);
#endif
  // Nothing left to trim?
  if (!shrinkAmount) {
#ifdef EMMALLOC_VERBOSE
    emm_diag_stow("emmalloc_trim(): Aligning for sbrk() requirements removed opportunity to trim.");
#endif
    return 0;
  }
...
#ifdef EMMALLOC_VERBOSE
    emm_diag_stow("emmalloc_trim(): Shrinking %zu bytes off the free heap end region. New free heap end region size: %zu bytes.", shrinkAmount, newRegionSize);
#endif
...
#ifdef EMMALLOC_VERBOSE
    emm_diag_stow("emmalloc_trim(): Created new free heap end region at %p. Size: %zu bytes.", lastActualRegion, newRegionSize);
#endif
  } else {
#ifdef EMMALLOC_VERBOSE
    emm_diag_stow("emmalloc_trim(): Not enough room to fit a free heap end region. Discarding it altogether.");
#endif
    newRegionSize = 0;
  }

  // Call sbrk() to shrink the memory area.
  void *oldSbrk = emm_sbrk_vary(-(int64_t)shrinkAmount);
  assert((intptr_t)oldSbrk != -1); // Shrinking with sbrk() should never fail.

  // Ask where sbrk got us at.
  void *sbrkNow = emm_sbrk_vary(0);
...
#ifdef EMMALLOC_VERBOSE
    emm_diag_stow("emmalloc_trim(): Created new sentinel end region at %p. Size: %zu bytes.", newEndSentinelRegion, newEndSentinelRegionSize);
#endif
...
void emmalloc_dump_free_dynamic_memory_fragmentation_map() {
  ...
  emm_diag_stow("numFreeMemoryRegions: %zu", numFreeMemoryRegions);
  for (int i = 0; i < 32; ++i) {
    emm_diag_stow("Free memory regions of size [%llu,%llu[ bytes: %zu regions", 1ull<<i, 1ull<<(i+1), freeMemorySizeMap[i]);
  }
}
