...
IMPORT Rd, Wr, Thread, Stdio, OSError, TextSeq, TextClass;
    ...
    IF n_args > 0 THEN
      ...
      TRY
        ...
        IF NOT t.doRecord THEN
          (* run the process *)
          TRY
            exit_code := System.Execute(command);
          EXCEPT
...
PROCEDURE FulfilExecPromise(ep : ExecPromise) : QPromise.ExitCode
  ...
  BEGIN
    ...
    TRY
      ...
      END;
      TRY
        exit_code := System.Execute(ep.cmd);
      EXCEPT
        System.ExecuteError(msg) => Err (ep.t, "execution failed: " & msg);
        exit_code := 1;
      END;
      FlushIO ();
