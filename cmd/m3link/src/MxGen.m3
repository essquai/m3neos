...
IMPORT M3IR, M3IR_Asm, TInt, TargetMap;
...
PROCEDURE GenerateMain (base: Mx.LinkSet;  c_output: Wr.T;  cg_output: M3IR.T;
...
PROCEDURE GenIRTypeDecls (VAR s: State) =
  VAR
    interface    : M3IR.Var;
  ...
  BEGIN
    ...
    s.cg.bind_segment (interface, Target.Address.bytes, struct_align,
                       Target.IRType.Struct, FALSE, TRUE);
...
PROCEDURE GenerateCGEntry (VAR s: State) =
  VAR
    ...
    addr_t    := Target.IRType.Addr;

  PROCEDURE GenAddUnits(ui: UnitInfo) =
    BEGIN
      WHILE (ui # NIL) DO
        s.cg.set_source_line (src_line);  INC (src_line);
        s.cg.start_call_direct (link_proc, 0, Target.IRType.Void);
        s.cg.load_procedure (ui.cg_proc);
        s.cg.pop_param (addr_t);
        s.cg.call_direct (link_proc, Target.IRType.Void);
        ui := ui.next;
      END;
    END GenAddUnits;
  ...
  PROCEDURE GenAddUnitImports(ui: UnitInfo) =
    BEGIN
      ...
      WHILE (ui # NIL) DO
        s.cg.set_source_line (src_line);  INC (src_line);
        s.cg.start_call_direct (link_proc2, 0, Target.IRType.Void);
        s.cg.load_procedure (ui.cg_proc);
        s.cg.pop_param (addr_t);
        s.cg.call_direct (link_proc2, Target.IRType.Void);
        ui := ui.next;
      END;
    END GenAddUnitImports;
  ...
  BEGIN
    ...
    s.cg.set_source_line (src_line);  INC (src_line);
    s.cg.start_call_direct (run_proc, 0, Target.IRType.Void);
    ...
    ELSE
      ...
      s.cg.pop_param (int_t);
    END;
    s.cg.call_direct (run_proc, Target.IRType.Void);
    ...
    (*  RTProcess.Exit (0); *)
    s.cg.set_source_line (src_line);  INC (src_line);
    s.cg.start_call_direct (exit_proc, 0, Target.IRType.Void);
    s.cg.load_integer (int_t, TInt.Zero);
    s.cg.pop_param (int_t);
    s.cg.call_direct (exit_proc, Target.IRType.Void);
...
PROCEDURE DeclareParam (VAR s: State;  nm: TEXT;  tipe: Target.IRType): M3IR.Var =
  BEGIN
    RETURN s.cg.declare_param (M3ID.Add (nm), TargetMap.CG_Bytes[tipe],
                               TargetMap.CG_Align_bytes [tipe],
                               tipe, (*typeUID*) 0, (*in_memory*) FALSE,
                               (*up_level*) FALSE, (*frequency*) M3IR.Always);
  END DeclareParam;

PROCEDURE DeclareLocal (VAR s: State;  nm: TEXT;  tipe: Target.IRType): M3IR.Var =
  BEGIN
