...
IMPORT RefSeq, TextSeq, Wr, Text, IntRefTbl, IntIntTbl;
IMPORT M3IR, M3IR_Asm, Target, TFloat, TargetMap, IntArraySort, Process;
IMPORT M3ID, TInt, TWord, ASCII, Thread, Stdio, Word, TextUtils;
FROM TargetMap IMPORT CG_Bytes, CG_Size;
FROM M3IR IMPORT Name, ByteOffset, CallingConvention;
FROM M3IR IMPORT BitSize, ByteSize, Alignment, Frequency;
FROM M3IR IMPORT Label, Sign, BitOffset, TypeUID;
FROM M3IR IMPORT Type, ZType, AType, RType, IType, MType;
FROM M3IR IMPORT CompareOp, ConvertOp, RuntimeError, MemoryOrder, AtomicOp;
FROM Target IMPORT IRType;
FROM M3IR_Asm IMPORT ErrorHandler;
IMPORT M3IR_MultiPass, M3IR_DoNothing, IRIO_Binary, RTIO;
IMPORT CharSeq, CharSeqRep, RTParams;
FROM M3CConv IMPORT IntToDec, IntToHex, UIntToHex, INT32;
...
TYPE Multipass_t = M3IR_MultiPass.T OBJECT
        self: T;
    OVERRIDES
...
T = M3IR_DoNothing.T OBJECT
    ...
    METHODS
        ...
        TIntLiteral(type: IRType; READONLY i: Target.Int): TEXT := TIntLiteral;
...
VAR BitsToCGInt := ARRAY BitSizeRange_t OF IRType { IRType.Void, .. };
VAR BitsToCGUInt := ARRAY BitSizeRange_t OF IRType { IRType.Void, .. };
VAR BitsToUInt := ARRAY BitSizeRange_t OF TEXT {NIL, ..};   (* "UINT8", "UINT16", "UINT32", "UINT64" *)
VAR SignedAndBitsToIRType: ARRAY BOOLEAN, BitSizeRange_t OF IRType;
...
PROCEDURE TypeText (self: T; cgtype: IRType; typeid: TypeUID; name: M3ID.T; type_text: TEXT := NIL): TEXT =
VAR type: Type_t := NIL;
  ...
  IF type_text = NIL THEN
    IF type # NIL THEN
      ...
      ELSE
        type_text := type.text & " ";
      END;
      IF cgtype = IRType.Addr AND NOT PassStructsByValue AND (type.isRecord() OR type.isArray()) THEN (* TODO remove this *)
        IF debug THEN
...
TYPE Expr_t = OBJECT
    ...
    points_to_cgtype: IRType := IRType.Void;
    refers_to_typeid: TypeUID := 0;
...
VAR type_text := self.type_text; (* TODO self.type.text *)
    ...
    rparen := "";
BEGIN
    Assert (self.self, cgtype # IRType.Void, "cgtype # IRType.Void");
    IF NOT force THEN
        ...
        *)
        ...
        ELSE
            ...
            lparen := "(";
            rparen := ")";
            IF left.cgtype = cgtype AND cgtype # IRType.Addr THEN
...
BEGIN
    IF FALSE THEN
        IF NOT ((cgtype = IRType.Void) # (type_text = NIL)) THEN
            RTIO.PutText("cgtype:" & cgtypeToText[cgtype] & " type_text:" & TextOrNil(type_text) & "\n");
            RTIO.Flush();
            <* ASSERT (cgtype = IRType.Void) # (type_text = NIL) *>
        END;
    END;
...
TYPE Var_t = M3IR.Var OBJECT
    ...
    type: Type_t := NIL;
    typeid: TypeUID := -1;
    points_to_cgtype: IRType; (* future *)
...
PROCEDURE VarNameT(v: M3IR.Var): TEXT =
BEGIN
    TYPECASE v OF
        NULL => RETURN "NIL"
...
TYPE Proc_t = M3IR.Proc OBJECT
    name: Name := 0;
    pending_output := FALSE;
    output := FALSE;
    op_start := 0; (* M3IR_MultiPass.Replay range *)
    op_end := 0;   (* M3IR_MultiPass.Replay range *)
...
VAR name := proc.name;
    ...
    is_common := proc.parent = NIL
                 ...
                 AND proc.level = 0;
    is_common_void := is_common AND proc.return_type = IRType.Void;
    ...
    *)
    ...
    proc.omit_prototype := is_setjmp OR is_alloca;
                           ...
                           - #include <setjmp.h> if there are any
                             instances of IRType.Jmpbuf
                           - render IRType.Jmpbuf as "jmp_buf" *)
...
"// Error 4739 catches m3front problems. See M3IR_MultiPass.TypeVersusSize.",
...
CONST intLiteralPrefix = ARRAY IRType OF TEXT {
    "",  "((INT8)",             (* 0 1 *)
    "",  "((INT16)",            (* 2 2 *)
    "", "",                     (* 4 3 *)
    "UINT64_(", "INT64_(",      (* 6 7 *)
...
CONST intLiteralSuffix = ARRAY IRType OF TEXT {
    "U",  ")",  (* 0 1 UINT8  INT8 *)
    "U",  ")",  (* 2 3 UINT16 INT16 *)
    "U", "",    (* 4 5 UINT32 INT32 *)
    ")", ")",   (* 6 7 UINT64 INT64 *)
...
CONST cgtypeToText = ARRAY IRType OF TEXT {
...
CONST cgtypeToParamText = ARRAY IRType OF TEXT {
...
TYPE IntegerTypes = [IRType.Word8 .. IRType.Int64];

CONST minMaxPossiblyValidForType = ARRAY IRType OF MinMaxBool_t {
...
CONST cgtypeIsUnsignedInt = ARRAY IRType OF BOOLEAN {
    TRUE, FALSE, (* 8 *)
    TRUE, FALSE, (* 16 *)
    TRUE, FALSE, (* 32 *)
    TRUE, FALSE, (* 64 *)
...
CONST cgtypeIsSignedInt = ARRAY IRType OF BOOLEAN {
    FALSE, TRUE, (* 8 *)
    FALSE, TRUE, (* 16 *)
    FALSE, TRUE, (* 32 *)
    FALSE, TRUE, (* 64 *)
...
<*UNUSED*>CONST cgtypeSizeBits = ARRAY IRType OF [0..64] {
    8, 8, 16, 16, 32, 32, 64, 64,
    32, 64, 64, (* real, longreal, extended *)
    0, 0, 0 (* address, struct, void *)
};
CONST cgtypeSizeBytes = ARRAY IRType OF [0..8] {
    1, 1, 2, 2, 4, 4, 8, 8,
...
CONST typeMinMax = ARRAY IRType OF MinMaxInt_t {
...
CONST typeToUnsigned = ARRAY IntegerTypes OF IntegerTypes {
    IRType.Word8, IRType.Word8,
    IRType.Word16, IRType.Word16,
    IRType.Word32, IRType.Word32,
    IRType.Word64, IRType.Word64
};
...
PROCEDURE push(self: T; <*UNUSED*>type: IRType; expression: Expr_t) =
BEGIN
...
PROCEDURE New0 (cfile: Wr.T): M3IR.T =
...
VAR widechar_target_type: Target.IRType;
    widechar_last := 0;
    type: Type_t := NIL;
BEGIN
    ...
    IF self.multipass.op_counts[IRIO_Binary.Op.widechar_size] > 0 THEN
      WITH op_widechar_size_list = self.multipass.op_data[IRIO_Binary.Op.widechar_size] DO
        TYPECASE op_widechar_size_list[LAST(op_widechar_size_list^)]
        OF NULL =>
        | M3IR_MultiPass.widechar_size_t (op_widechar_size) =>
...
*)
    ...
    IF NUMBER(multipass.op_data[IRIO_Binary.Op.case_jump]^) > 0 THEN
...
BEGIN
    ...
    M3IR_MultiPass.end_unit(self); (* let M3IR_MultiPass do its usual last step *)
    self.Replay(x, index, self.op_data[IRIO_Binary.Op.begin_unit]);
    self.Replay(x, index, self.op_data[IRIO_Binary.Op.set_error_handler]);
    Prefix_Print(x, self);
...
TYPE DeclareTypes_t = M3IR_DoNothing.T OBJECT
    self: T := NIL;
...
PROCEDURE SubrangeIRType(READONLY min, max: Target.Int; bit_size: BitSize): IRType =
(* slightly strange logic -- see m3front/src/types/SubrangeType
   m3front should pass us down cgtype directly, and not
   bother with bit_size, domain *)
BEGIN
    RETURN SignedAndBitsToIRType[SubrangeIsSigned(min, max)][bit_size];
END SubrangeIRType;
...
BEGIN
    ...
    x.Type_Init(NEW (Subrange_t,
        ...
        bit_size := bit_size,
        cgtype := SubrangeIRType(min, max, bit_size)),
...
PROCEDURE declare_exception(self: DeclareTypes_t; name: Name; arg_type: TypeUID; raise_proc: BOOLEAN; base: M3IR.Var; offset: INTEGER) =
VAR x := self.self;
BEGIN
    IF debug_verbose THEN
...
PROCEDURE set_runtime_proc(multipass: Multipass_t; name: Name; <*UNUSED*>p: M3IR.Proc) =
...
PROCEDURE import_global (declareTypes: DeclareTypes_t; name: Name; byte_size: ByteSize; alignment: Alignment; cgtype: IRType; typeid: TypeUID; typename: Name): M3IR.Var =
...
PROCEDURE Imports_import_global (imports: Imports_t; name: Name; byte_size: ByteSize; alignment: Alignment; cgtype: IRType; typeid: TypeUID; typename: Name): M3IR.Var =
...
PROCEDURE Locals_declare_segment(self: Locals_t; name: Name; typeid: TypeUID; const: BOOLEAN): M3IR.Var =
BEGIN
    RETURN declare_segment(self.self, name, typeid, const);
END Locals_declare_segment;

PROCEDURE declare_segment(self: T; name: Name; typeid: TypeUID; const: BOOLEAN): M3IR.Var =
...
TYPE CountUsedLabels_t = M3IR_DoNothing.T OBJECT
    count := 0;
OVERRIDES
    case_jump := CountUsedLabels_case_jump;
END;

TYPE MarkUsed_t = M3IR_DoNothing.T OBJECT
    self: T;
...
TYPE Segments_t = M3IR_DoNothing.T OBJECT
    ...
    FOR i := FIRST(setData) TO LAST(setData) DO
        IF self.op_counts[setData[i].op] > 0 THEN
            IF NOT setAny THEN
                ...
                setAny := TRUE;
            END;
            ifndef(x, "m3set" & IRIO_Binary.OpText(setData[i].op), "//4");
...
TYPE HelperFunctions_t = M3IR_DoNothing.T OBJECT
    self: T := NIL;
    data: RECORD
        ...
        insert, sign_extend, pop, check_range, xor := SET OF IRType{};
        compare := ARRAY CompareOp OF SET OF IRType{ SET OF IRType{}, .. };
    END;
METHODS
...
PROCEDURE HelperFunctions_helper_with_type_and_array(
    self: HelperFunctions_t; op: TEXT; type: IRType; VAR types_already_printed: SET OF IRType; READONLY first: ARRAY OF TEXT) =
BEGIN
    (* Print the prefix array before any type-specific content. *)
    IF types_already_printed = SET OF IRType{} THEN
    ...
    IF NOT type IN types_already_printed THEN
        ...
        types_already_printed := types_already_printed + SET OF IRType{type};
    END;
END HelperFunctions_helper_with_type_and_array;

PROCEDURE HelperFunctions_helper_with_type(self: HelperFunctions_t; op: TEXT; type: IRType; VAR types_already_printed: SET OF IRType; first: TEXT := NIL) =
BEGIN
...
PROCEDURE HelperFunctions_pop(self: HelperFunctions_t; type: IRType) =
...
TYPE Locals_t = M3IR_DoNothing.T OBJECT
    self: T := NIL;
OVERRIDES
...
PROCEDURE AllocateTemps_common(self: AllocateTemps_t; type: IRType) =
VAR x := self.self;
BEGIN
...
BEGIN
    ...
    AllocateTemps_common(self, IRType.Addr);
...
TYPE Imports_t = M3IR_DoNothing.T OBJECT
    self: T;
OVERRIDES
...
PROCEDURE Imports_import_procedure(
    ...
    return_typename: Name): M3IR.Proc =
BEGIN
    ReplaceName (self.self, name);
...
TYPE GetStructSizes_t = M3IR_DoNothing.T OBJECT
    sizes: REF ARRAY OF INTEGER := NIL;
    count := 0;
METHODS
    Declare(type: IRType; byte_size: ByteSize; alignment: Alignment): M3IR.Var := GetStructSizes_Declare;
OVERRIDES
...
CONST Ops = ARRAY OF IRIO_Binary.Op{
    (* These are all the operations that can introduce a struct size. *)
        IRIO_Binary.Op.declare_constant,
        IRIO_Binary.Op.declare_global,
        IRIO_Binary.Op.declare_local,
        IRIO_Binary.Op.declare_param,
        IRIO_Binary.Op.declare_temp,
        IRIO_Binary.Op.import_global};
...
PROCEDURE GetStructSizes_Declare(self: GetStructSizes_t; type: IRType; byte_size: ByteSize; alignment: Alignment): M3IR.Var =
BEGIN

    <* ASSERT byte_size >= 0 *>
    <* ASSERT (byte_size MOD alignment) = 0 *>

    IF type = IRType.Struct THEN
...
BEGIN
    ...
    END;
    IF var.cgtype # IRType.Struct THEN
...
BEGIN
    ...
    IF PassStructsByValue AND var.cgtype = IRType.Struct AND var.up_level THEN
        struct := "*";
    END;
...
VAR struct := "";
BEGIN
    IF var.cgtype = IRType.Struct AND NOT PassStructsByValue THEN
...
VAR struct := " ";
BEGIN
    IF NOT PassStructsByValue AND var.cgtype = IRType.Struct THEN
        struct := "*";
    END;
...
BEGIN
    ...
    IF proc.add_static_link THEN
        param := NARROW(internal_declare_param(
            self,
            self.static_link_id,
            CG_Bytes[IRType.Addr], (* size *)
            CG_Bytes[IRType.Addr], (* alignment *)
            IRType.Addr,
            -1, (* typeid *)
...
(* returns derived Var_t instead of base M3IR.Var *)
    self: T;
    byte_size: ByteSize;
    alignment: Alignment;
    type: IRType): Var_t =
...
PROCEDURE free_temp(self: T; <*NOWARN*>v: M3IR.Var) =
BEGIN
...
PROCEDURE begin_init(self: T; <*UNUSED*>v: M3IR.Var) =
BEGIN
...
PROCEDURE Segments_begin_init(self: Segments_t; v: M3IR.Var) =
BEGIN
...
PROCEDURE end_init(self: T; v: M3IR.Var) =
VAR var := NARROW(v, Var_t);
    fields := self.fields;
...
PROCEDURE Segments_end_init(self: Segments_t; v: M3IR.Var) =
BEGIN
...
PROCEDURE init_helper(self: T; offset: ByteOffset; type: IRType) =
BEGIN
(*
    IF debug_verbose THEN
...
PROCEDURE adapt_c_init_type(READONLY value: Target.Int; type: IRType): IRType =
...
PROCEDURE init_proc(self: T; offset: ByteOffset; p: M3IR.Proc) =
...
BEGIN
    ...
    END;
    init_helper(self, offset, IRType.Addr); (* FUTURE: better typing *)
    initializer_addhi(self, "(ADDRESS)&" & NameT(proc.name));
END init_proc;

PROCEDURE MarkUsed_proc(self: T; p: M3IR.Proc) =
VAR proc: Proc_t;
BEGIN
...
PROCEDURE Segments_init_proc(self: Segments_t; offset: ByteOffset; p: M3IR.Proc) =
BEGIN
...
PROCEDURE init_var(self: T; offset: ByteOffset; v: M3IR.Var; bias: ByteOffset) =
VAR var := NARROW(v, Var_t);
    bias_text := "";
BEGIN
    IF debug_verbose THEN
...
PROCEDURE Segments_init_var(self: Segments_t; offset: ByteOffset; v: M3IR.Var; bias: ByteOffset) =
BEGIN
    init_var(self.self, offset, v, bias);
END Segments_init_var;

PROCEDURE init_offset(self: T; <*UNUSED*>offset: ByteOffset; <*UNUSED*>value: M3IR.Var) =
BEGIN
    self.comment("init_offset");
    <* ASSERT FALSE *>
END init_offset;

PROCEDURE Segments_init_offset(self: Segments_t; offset: ByteOffset; value: M3IR.Var) =
BEGIN
...
BEGIN
    ...
    IF length # 0 THEN
      ...
      FOR i := 0 TO length - 1 DO
        init_helper(self, offset + i, IRType.Word8);
...
PROCEDURE TIntLiteral(self: T; type: IRType; READONLY i: Target.Int): TEXT =
<*FATAL Wr.Failure, Thread.Alerted*>
VAR ok1 := TRUE;
    ok2 := TRUE;
BEGIN
    CASE type OF
        | IRType.Int8   => ok1 := TInt.GE(i, TInt.Min8 );
                         (*ok2 := TInt.LE(i, TInt.Max8);*)
                           ok2 := TInt.LE(i, TWord.Max8);
        | IRType.Int16  => ok1 := TInt.GE(i, TInt.Min16);
                         (*ok2 := TInt.LE(i, TInt.Max16);*)
                           ok2 := TInt.LE(i, TWord.Max16);
        | IRType.Int32  => ok1 := TInt.GE(i, TInt.Min32);
                         (*ok2 := TInt.LE(i, TInt.Max32);*)
                           ok2 := TInt.LE(i, TWord.Max32);
        | IRType.Int64  => ok1 := TInt.GE(i, TInt.Min64);
                         (*ok2 := TInt.LE(i, TInt.Max64);*)
                           ok2 := TInt.LE(i, TWord.Max64);
        | IRType.Word8  => ok1 := TWord.LE(i, TWord.Max8);
        | IRType.Word16 => ok1 := TWord.LE(i, TWord.Max16);
        | IRType.Word32 => ok1 := TWord.LE(i, TWord.Max32);
        | IRType.Word64 => ok1 := TWord.LE(i, TWord.Max64);
        ELSE
    ...
    IF NOT ok1 OR NOT ok2 THEN
        ...
        (* ASSERT ok2 *)
    END;
    IF type = IRType.Int32 AND TInt.EQ(i, TInt.Min32) THEN
        RETURN "-" & intLiteralPrefix[type] & TInt.ToText(TInt.Max32) & intLiteralSuffix[type] & "-1";
    ELSIF type = IRType.Int64 AND TInt.EQ(i, TInt.Min64) THEN
...
PROCEDURE IntLiteral(self: T; type: IRType; i: INTEGER): TEXT =
BEGIN
...
PROCEDURE import_procedure(
    ...
    return_type: IRType; callingConvention: CallingConvention;
    return_typeid: TypeUID;
    return_typename: Name): M3IR.Proc =
VAR proc: Proc_t := NIL;
BEGIN
...
PROCEDURE ProcNameOrNIL(proc: M3IR.Proc): TEXT =
BEGIN
    IF proc = NIL THEN
        RETURN "NIL";
    END;
...
PROCEDURE declare_procedure (
  ...
  parameter_count: INTEGER; return_type: IRType;
  level: INTEGER; callingConvention: CallingConvention;
  exported: BOOLEAN; parent: M3IR.Proc;
  return_typeid: TypeUID; return_typename: Name): M3IR.Proc =
...
PROCEDURE Locals_begin_procedure (locals: Locals_t; p: M3IR.Proc) =
VAR self := locals.self;
    proc := NARROW(p, Proc_t);
BEGIN
    IF debug_verbose THEN
...
PROCEDURE Locals_end_procedure (locals: Locals_t; p: M3IR.Proc) =
VAR self := locals.self;
BEGIN
...
PROCEDURE begin_procedure(self: T; p: M3IR.Proc) =
VAR proc := NARROW(p, Proc_t);
...
BEGIN
    ...
    FOR i := 0 TO proc.Locals_Size() - 1 DO
        WITH local = proc.Locals(i) DO
            ...
            IF (NOT local.up_level) AND local.used THEN
                ...
                ELSIF local.cgtype = IRType.Struct THEN
                  print(self, "={0};//struct\n");
    ...
    IF NOT PassStructsByValue THEN
        FOR i := FIRST(params^) TO LAST(params^) DO
            WITH param = params[i] DO
                IF (NOT param.up_level) AND (param.cgtype = IRType.Struct) AND param.used THEN
    ...
    IF proc.forward_declared_frame_type THEN
        FOR i := FIRST(params^) TO LAST(params^) DO
            WITH param = params[i] DO
                IF param.up_level AND param.used THEN
                    struct := "";
                    IF param.cgtype = IRType.Struct THEN
    ...
    FOR i := FIRST(params^) TO LAST(params^) DO
        WITH param = params[i] DO
            IF (NOT PassStructsByValue) AND (NOT param.up_level) AND param.cgtype = IRType.Struct AND param.used THEN
     ...
     *)
    proc.needs_return := (proc.return_type # IRType.Void);
END begin_procedure;

PROCEDURE end_procedure(self: T; p: M3IR.Proc) =
VAR proc := NARROW(p, Proc_t);
BEGIN
...
PROCEDURE note_procedure_origin(self: T; <*UNUSED*>p: M3IR.Proc) =
BEGIN
...
PROCEDURE exit_proc(self: T; type: IRType) =
...
BEGIN
    ...
    IF proc.no_return THEN
        ...
        <* ASSERT type = IRType.Void  *>
        INC(proc.exit_proc_skipped);
        RETURN;
    END;
    proc.needs_return := FALSE;
    IF type = IRType.Void THEN
        IF NOT proc.is_RTHooks_Raise THEN
            print(self, "return;\n");
        END;
    ELSE
        (* TODO Is the cast avoidable? *)
        IF type = IRType.Addr OR (NOT ReturnStructsByValue AND type = IRType.Struct) THEN
            cast1 := "(";
...
PROCEDURE load(self: T; v: M3IR.Var; offset: ByteOffset; in_mtype: MType; out_ztype: ZType) =
...
BEGIN
    ...
    IF offset # 0 OR var.cgtype # in_mtype THEN
        expr := AddressOf(expr);
        IF offset # 0 THEN
            expr := cast(expr, type := IRType.Addr);
            expr := NEW(Expr_t, right := expr, left := IntToExpr(self, offset), c_binop_text := "+");
        END;
        expr := cast(expr, type := IRType.Addr, type_text := cgtypeToText[in_mtype] & "*");
        expr := Deref(expr);
    END;
...
PROCEDURE store(self: T; v: M3IR.Var; offset: ByteOffset; ztype: ZType; mtype: MType) =
...
PROCEDURE load_address(self: T; v: M3IR.Var; offset: ByteOffset) =
...
BEGIN
    ...
    pop(self);
    IF offset # 0 THEN
        expr := cast(expr, type := IRType.Addr); (* might be redundant *)
        expr := NEW(Expr_t, right := expr, left := IntToExpr(self, offset), c_binop_text := "+");
    END;
    expr := CastAndDeref(expr, type := IRType.Addr, type_text := cgtypeToText[mtype] & "*"); (* cast might be redundant *)
    IF mtype # ztype THEN
...
BEGIN
    ...
    END;
    push(self, IRType.Addr, CTextToExpr("0")); (* UNDONE NULL or (ADDRESS)0? *)
END load_nil;
...
BEGIN
    ...
    expr.cgtype := type;
    IF cgtypeIsUnsignedInt[type] THEN
        match_readonlyi := TWord.Truncate(readonly_i, size, i)
    ELSIF cgtypeIsSignedInt[type] THEN
        match_readonlyi := TInt.Extend(readonly_i, size, i)
    END;
    <* ASSERT match_readonlyi *> 
    (* Sunil: Assertion below replaced with the one above
              and hack module TIntN replaced with TWord/TInt above *)
    (* <* ASSERT readonly_i = i *> *)
    expr.minmax[Min] := i;
    expr.minmax[Max] := i;
    expr := cast(expr, type);
...
PROCEDURE cast(expr: Expr_t; type: IRType := IRType.Void; type_text: TEXT := NIL): Expr_t =
VAR e := NEW(Expr_Cast_t, cgtype := type, type_text := type_text, left := expr);
BEGIN
    <* ASSERT type # IRType.Void *>
    (* ASSERT (type = IRType.Void) # (type_text = NIL) *)
    (* casts are either truncating or sign extending or zero extending *)
    TransferMinMax(expr, e);
    RETURN e;
END cast;

<*UNUSED*>PROCEDURE old_Cast(expr: Expr_t; type: IRType := IRType.Void; type_text: TEXT := NIL): Expr_t =
BEGIN
    <* ASSERT (type = IRType.Void) # (type_text = NIL) *>
    RETURN NEW(Expr_Cast_t, cgtype := type, type_text := type_text, left := expr);
END old_Cast;

PROCEDURE CastAndDeref(expr: Expr_t; type: IRType := IRType.Void; type_text: TEXT := NIL): Expr_t =
BEGIN
    <* ASSERT type # IRType.Void *>
    RETURN Deref(cast(expr, type, type_text));
END CastAndDeref;

PROCEDURE op1(self: T; type: IRType; name, op: TEXT) =
(* unary operation *)
...
<* UNUSED *> PROCEDURE TIntExtendOrTruncate(READONLY in: Target.Int; type: IRType; VAR out: Target.Int): BOOLEAN =
...
VAR s0 := cast(get(self, 0), IRType.Addr);
    s1 := cast(get(self, 1), IRType.Addr);
    s2 := cast(get(self, 2), IRType.Addr);
...
VAR s0 := cast(get(self, 0), type);
    s1 := cast(get(self, 1), IRType.Addr, "SET");
BEGIN
    IF debug THEN
...
VAR swap := (op IN SET OF CompareOp{CompareOp.GT, CompareOp.GE});
    s0 := cast(get(self, ORD(swap)), IRType.Addr, "SET");
    s1 := cast(get(self, ORD(NOT swap)), IRType.Addr, "SET");
    target_word_bytes := Target.Word.bytes;
...
VAR s0 := cast(get(self, 0), type);
    s1 := cast(get(self, 1), type);
    s2 := cast(get(self, 2), IRType.Addr, "SET");
BEGIN
    IF debug THEN
...
PROCEDURE shift_or_rotate(self: T; type: IType; which: TEXT; count_type: IRType) =
...
PROCEDURE swap(self: T; <*UNUSED*>a, b: IRType) =
(* tmp := s1; s1 := s0; s0 := tmp *)
VAR temp := get(self, 1);
BEGIN
    IF debug THEN
...
PROCEDURE cg_pop(self: T; type: IRType) =
...
PROCEDURE loophole(self: T; from: ZType; to: ZType) =
  ...
  VAR
    cast, s0: Expr_t;
  BEGIN
    (* As noted in the documentation from M3IR_Asm, this is a
...
BEGIN
    ...
    END;
    self.store(t, 0, IRType.Addr, IRType.Addr);
    self.load(t, 0, IRType.Addr, IRType.Addr);
    IF debug THEN
...
(* 32: see M3IR.RuntimeError, RuntimeError.T *)
...
BEGIN
    ...
    END;
    pop(self);
    push(self, IRType.Addr, address_plus_offset(s0.CText(), offset));
END add_offset;
...
BEGIN
    ...
    ELSE
        ...
        END;
        push(self, IRType.Addr, paren(CTextToExpr(s1.CText() & "+" & paren(s0).CText())));
    END;
END index_address;
...
PROCEDURE MarkUsed_start_call_direct(self: MarkUsed_t;
    p: M3IR.Proc; <*UNUSED*>level: INTEGER; <*UNUSED*>type: IRType) =
BEGIN
    MarkUsed_proc(self.self, p);
END MarkUsed_start_call_direct;

PROCEDURE start_call_direct(self: T; p: M3IR.Proc; <*UNUSED*>level: INTEGER; type: IRType) =
...
BEGIN
    ...
    IF proc.is_exception_handler THEN
        load_nil (self);
        pop_param (self, IRType.Addr);
    END;
END start_call_direct;

PROCEDURE start_call_indirect(self: T; <*UNUSED*>type: IRType; <*UNUSED*>callingConvention: CallingConvention) =
(* begin a procedure call to a procedure at static level 'level'. *)
BEGIN
    IF debug THEN
...
BEGIN
    ...
    s0 := cast(s0, type := IRType.Addr, type_text := type_text);

    <* ASSERT NOT PassStructsByValue *> (* needs work *)
    IF PassStructsByValue THEN
        s0 := Deref(s0);
    ELSE
      s0.cgtype := IRType.Addr; (* set type for indirect call function pointer cast *)
    END;
...
BEGIN
    ...
    self.store(var, 0, IRType.Addr, IRType.Addr);
END pop_static_link;

PROCEDURE Locals_pop_static_link(self: Locals_t) =
VAR x := self.self;
    var := internal_declare_temp(x, CG_Bytes[IRType.Addr], CG_Bytes[IRType.Addr], IRType.Addr);
BEGIN
...
PROCEDURE call_helper(self: T; type: IRType; proc: TEXT) =
    ...
    ELSIF direct THEN (* TODO? No casts for indirect calls? *)
      ...
      ELSE
        WITH param = types[index] DO
          IF param.cgtype # IRType.Struct OR param.typeid = UID_ADDR THEN
            t1 := " (";
            (* TODO type.text *)
            t2 := " " & param.type_text;
            t3 := " )(";
...
BEGIN
    ...
    e.cgtype := IRType.Addr; (* TODO better typing *)
    RETURN e;
END get_static_link;

PROCEDURE call_direct(self: T; p: M3IR.Proc; type: IRType) =
(* call the procedure identified by M3IR.Proc p. The procedure
...
PROCEDURE call_indirect(self: T; type: IRType; <*UNUSED*>callingConvention: CallingConvention) =
...
<*NOWARN*>PROCEDURE invoke_direct (self : T; p : M3IR.Proc; t : IRType; next,handler : Label) =
  BEGIN
    IF debug THEN
...
<*NOWARN*>PROCEDURE invoke_indirect (self : T; t : IRType; cc : CallingConvention; next,handler : Label) =
  BEGIN
    IF debug THEN
...
<*NOWARN*>PROCEDURE landing_pad (self : T; t : ZType; handler : Label; READONLY catches : ARRAY OF TypeUID) =
  BEGIN
    ...
    END;
    push(self, IRType.Addr, CTextToExpr("0"));
  END landing_pad;

(*------------------------------------------- procedure and closure types ---*)

PROCEDURE MarkUsed_load_procedure(self: MarkUsed_t; p: M3IR.Proc) =
BEGIN
    MarkUsed_proc(self.self, p);
END MarkUsed_load_procedure;

PROCEDURE load_procedure(self: T; p: M3IR.Proc) =
...
BEGIN
    ...
    END;
    (* UNDONE? typeing? *)
    push(self, IRType.Addr, CTextToExpr(NameT(proc.name)));
END load_procedure;

PROCEDURE load_static_link(self: T; p: M3IR.Proc) =
...
BEGIN
    ...
    IF target.level = 0 THEN
        ...
        RETURN;
    END;
    push(self, IRType.Addr, get_static_link(self, target));
...
BEGIN
    BitsToCGUInt[8] := IRType.Word8;
    BitsToCGUInt[16] := IRType.Word16;
    BitsToCGUInt[32] := IRType.Word32;
    BitsToCGUInt[64] := IRType.Word64;
    BitsToCGInt[8] := IRType.Int8;
    BitsToCGInt[16] := IRType.Int16;
    BitsToCGInt[32] := IRType.Int32;
    BitsToCGInt[64] := IRType.Int64;
    BitsToUInt[8] := Text_uint8;
    BitsToUInt[16] := Text_uint16;
    BitsToUInt[32] := Text_uint32;
    BitsToUInt[64] := Text_uint64;
    SignedAndBitsToIRType[TRUE] := BitsToCGInt;
    SignedAndBitsToIRType[FALSE] := BitsToCGUInt;
