...
IMPORT M3ID, M3IR, M3IR_Asm, Target, TargetMap;

FROM M3IR IMPORT Name, ByteOffset, CallingConvention;
FROM M3IR IMPORT ByteSize, Alignment, Frequency, RuntimeError;
FROM M3IR IMPORT Var, Proc, Label, Sign, CompareOp, ConvertOp, AtomicOp;
FROM M3IR IMPORT Type, ZType, AType, RType, IType, MType;
FROM M3IR IMPORT MemoryOrder, TypeUID;
...
TYPE
  U = M3IR.T OBJECT
        ...
        init_cursor  := 0;
invoke := FALSE;
        note_error   : M3IR_Asm.ErrorHandler := NIL;
...
PROCEDURE PutErr (u: U;  a, b, c: TEXT := NIL) =
  BEGIN
    ...
    u.note_error ("********* M3IR_Check ERROR *********** " & a & b & c);
    u.child.comment ("********* M3IR_Check ERROR *********** ", a, b, c);
    INC (u.n_errors);
  END PutErr;
...
PROCEDURE New (child: M3IR.T;
               clean_jumps, clean_stores: BOOLEAN;
               nested_calls, nested_procs: BOOLEAN): M3IR.T =
  BEGIN
...
PROCEDURE CrashAndBurn (msg: TEXT) =
  ...
  BEGIN
    Wr.PutText (Stdio.stdout, "Unhandled M3IR_Check error: " & msg);
...
PROCEDURE set_error_handler (self: U;  p: M3IR_Asm.ErrorHandler) =
  BEGIN
    self.note_error := p;
...
*)
    v := self.child.declare_temp (s, a, t, m3t, in_memory, typename);
(*
...
(*                           ^ This was T_to_ST [u], which fails to account
                               ...
                               in M3IR_Asm seem to suggest it is passed by value.
