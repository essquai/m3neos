...
IMPORT M3ID, M3IR, M3IR_Asm, IRIO_Binary;
IMPORT Target, TargetMap, TInt, TFloat, TWord;
FROM M3IR IMPORT CompareOp, ConvertOp, AtomicOp, RuntimeError, MemoryOrder;
...
TYPE
  State = RECORD
    ...
    buf_ptr: CARDINAL;
    hit_eof: BOOLEAN;
    vars   : REF ARRAY OF M3IR.Var;
    procs  : REF ARRAY OF M3IR.Proc;
    labels : REF ARRAY OF M3IR.Label;
  END;
...
PROCEDURE Inhale (rd: Rd.T;  cg: M3IR.T) =
  VAR s: State;  op: M3IR.Name;  cmd: INTEGER;
  BEGIN
    s.rd      := rd;
    s.cg      := cg;
    ...
    s.buf_ptr := 0;
    s.hit_eof := FALSE;
    s.vars    := NEW (REF ARRAY OF M3IR.Var, 400);
    s.procs   := NEW (REF ARRAY OF M3IR.Proc, 50);
    s.labels  := NEW (REF ARRAY OF M3IR.Label, 400);

    FOR i := 0 TO LAST(s.labels^) DO s.labels[i] := M3IR.No_label END;

    cmd := Scan_int (s);
    IF (cmd # IRIO_Binary.Version) THEN
      Error (s, "** bad intermediate code version: expected ",
                Fmt.Int (IRIO_Binary.Version) & ", but found ",
                Fmt.Int (cmd));
    END;
...
PROCEDURE Error (<*UNUSED*> VAR s: State;  a, b, c: TEXT := NIL) =
  ...
  VAR msg := Wr.EOL & "** ERROR in IRIO_BinRd.Inhale: ";
  BEGIN
...
PROCEDURE Scan_name (VAR s: State): M3IR.Name =
  VAR txt := Scan_text (s);
  BEGIN
    IF (txt = NIL)
...
PROCEDURE Scan_type (VAR s: State): M3IR.Type =
  VAR x := GetByte (s);
  BEGIN
    IF (ORD (FIRST (M3IR.Type)) <= x) AND (x <= ORD (LAST (M3IR.Type))) THEN
      RETURN VAL (x, M3IR.Type);
    ELSE
      Error (s, "illegal type: ", Fmt.Int (x));
      RETURN M3IR.Type.Int32;
    END;
  END Scan_type;
...
PROCEDURE Scan_label (VAR s: State): INTEGER =
  ...
  BEGIN
    val := Scan_int (s);
    IF (val < 0) THEN RETURN M3IR.No_label; END;

    WHILE (val > LAST (s.labels^)) DO ExpandLabels (s) END;

    x := s.labels[val];
    IF (x = M3IR.No_label) THEN
      x := s.cg.next_label ();
      s.labels[val] := x;
    END;
    RETURN x;
...
PROCEDURE ExpandLabels (VAR s: State) =
  VAR new := NEW (REF ARRAY OF M3IR.Label, 2 * NUMBER (s.labels^));
  BEGIN
    SUBARRAY (new^, 0, NUMBER (s.labels^)) := s.labels^;
    FOR i := NUMBER (s.labels^) TO LAST (new^) DO new[i] := M3IR.No_label END;
    s.labels := new;
  END ExpandLabels;

PROCEDURE Scan_tipe (VAR s: State): M3IR.TypeUID =
  BEGIN
    RETURN Scan_int (s);
  END Scan_tipe;

PROCEDURE Scan_var (VAR s: State): M3IR.Var =
  VAR id := Scan_int (s);
  BEGIN
    IF (id <= 0)
      THEN RETURN NIL;
...
PROCEDURE Scan_proc (VAR s: State): M3IR.Proc =
  VAR id := Scan_int (s);
  BEGIN
    IF (id <= 0)
      THEN RETURN NIL;
...
PROCEDURE Scan_sign (VAR s: State): M3IR.Sign =
  VAR x := Scan_int (s);
  BEGIN
    CASE x OF
    | ORD (M3IR.Sign.Positive) => RETURN M3IR.Sign.Positive;
    | ORD (M3IR.Sign.Negative) => RETURN M3IR.Sign.Negative;
    | ORD (M3IR.Sign.Unknown)  => RETURN M3IR.Sign.Unknown;
    ELSE Error (s, "bad sign: ", Fmt.Int (x));
    END;
    RETURN M3IR.Sign.Unknown;
  END Scan_sign;
...
PROCEDURE Scan_Tint (VAR s: State): Target.Int =
  ...
  BEGIN
    CASE i OF
    | IRIO_Binary.Int1  =>  n_bytes := 1;  sign := +1;
    | IRIO_Binary.NInt1 =>  n_bytes := 1;  sign := -1;
    | IRIO_Binary.Int2  =>  n_bytes := 2;  sign := +1;
    | IRIO_Binary.NInt2 =>  n_bytes := 2;  sign := -1;
    | IRIO_Binary.Int4  =>  n_bytes := 4;  sign := +1;
    | IRIO_Binary.NInt4 =>  n_bytes := 4;  sign := -1;
    | IRIO_Binary.Int8  =>  n_bytes := 8;  sign := +1;
    | IRIO_Binary.NInt8 =>  n_bytes := 8;  sign := -1;
    ELSE
...
PROCEDURE Scan_int (VAR s: State): INTEGER =
  ...
  BEGIN
    CASE i OF
    | IRIO_Binary.Int1  =>  RETURN GetByte (s);
    | IRIO_Binary.NInt1 =>  RETURN - GetByte (s);
    | IRIO_Binary.Int2  =>  n_bytes := 2;  sign := +1;
    | IRIO_Binary.NInt2 =>  n_bytes := 2;  sign := -1;
    | IRIO_Binary.Int4  =>  n_bytes := 4;  sign := +1;
    | IRIO_Binary.NInt4 =>  n_bytes := 4;  sign := -1;
...
PROCEDURE AddVar (VAR s: State;  id: INTEGER;  v: M3IR.Var) =
  BEGIN
...
PROCEDURE ExpandVars (VAR s: State) =
  VAR new := NEW (REF ARRAY OF M3IR.Var, 2 * NUMBER (s.vars^));
  BEGIN
...
PROCEDURE AddProc (VAR s: State;  id: INTEGER;  p: M3IR.Proc) =
  BEGIN
...
PROCEDURE ExpandProcs (VAR s: State) =
  VAR new := NEW (REF ARRAY OF M3IR.Proc, 2 * NUMBER (s.procs^));
  BEGIN
...
PROCEDURE import_procedure (VAR s: State) =
  VAR name     := Scan_name (s);
      ...
      p        := Scan_int (s);
      (* Note: return_typeid restored, return_typename nobody uses *)
  BEGIN
...
PROCEDURE declare_procedure (VAR s: State) =
  VAR name     := Scan_name (s);
      ...
      return_typeid := Scan_tipe(s);
      p        := Scan_int (s);
      (* Note: return_typeid restored to IR;
         return_typename remains unused downstream and so omitted *)
  BEGIN
...
PROCEDURE case_jump (VAR s: State) =
  VAR type := Scan_type (s);
      n    := Scan_int (s);
      x    := NEW (REF ARRAY OF M3IR.Label, n);
  BEGIN
...
PROCEDURE landing_pad (VAR s: State) =
  VAR type    := Scan_type (s);
      handler := Scan_label (s);
      n       := Scan_int (s);
      x       := NEW (REF ARRAY OF M3IR.TypeUID, n);
  BEGIN
