MODULE M3IR_MultiPass;

IMPORT M3IR, M3IR_Asm, RefSeq, Target;
FROM M3IR IMPORT Type, MType, IType, RType, AType, ZType, Sign;
FROM M3IR IMPORT Name, Var, Proc, Alignment, TypeUID, Label;
FROM M3IR IMPORT Frequency, CallingConvention, CompareOp, ConvertOp, AtomicOp;
FROM M3IR IMPORT BitSize, ByteSize, BitOffset, ByteOffset, RuntimeError;
FROM M3IR IMPORT MemoryOrder;
FROM IRIO_Binary IMPORT Op;
...
REVEAL
T = Public BRANDED "M3IR_MultiPass.T" OBJECT
...
REVEAL Replay_t = BRANDED "M3IR_MultiPass.Replay_t" OBJECT
    refs: StaticRefs_t; (* vars and procs *)
    reuse_refs := FALSE;
METHODS
    Init(refcount: INTEGER): Replay_t := Replay_Init;
    PutRef(tag: INTEGER; ref: REFANY) := Replay_PutRef;
    GetVar(ref: INTEGER): M3IR.Var := Replay_GetVar;
    GetProc(ref: INTEGER): M3IR.Proc := Replay_GetProc;
END;
...
PROCEDURE TypeVersusSize (VAR cgtype: IRType; byte_size: ByteSize) =
BEGIN
   ...
   *)
  IF cgtype # IRType.Struct AND byte_size # CG_Bytes [cgtype] THEN
    cgtype := IRType.Struct;
  END;
END TypeVersusSize;
...
PROCEDURE Replay_GetProc(self: Replay_t; ref: INTEGER): M3IR.Proc =
VAR proc := self.refs.data[ref];
BEGIN
    IF proc = NIL THEN
        RETURN NIL;
    END;
    RETURN NARROW(proc, M3IR.Proc);
END Replay_GetProc;

PROCEDURE Replay_GetVar(self: Replay_t; ref: INTEGER): M3IR.Var =
VAR var := self.refs.data[ref];
BEGIN
    IF var = NIL THEN
        RETURN NIL;
    END;
    RETURN NARROW(var, M3IR.Var);
END Replay_GetVar;
...
PROCEDURE replay_fetch_and_op(self: fetch_and_op_t; <*UNUSED*>replay: Replay_t; cg: M3IR.T) = BEGIN cg.fetch_and_op(self.atomic_op, self.mtype, self.ztype, self.order); END replay_fetch_and_op;
