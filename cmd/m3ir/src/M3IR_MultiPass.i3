INTERFACE M3IR_MultiPass;

(* This persists a run of M3IR calls to memory, so that multiple
 ...
 * 3 You can implement "mini" M3IRs and pass them to call(). This lets you split up your code more.
 * I expect to use #3.
 *)

IMPORT M3IR, Target, M3IR_AssertFalse, M3ID;
FROM M3IR IMPORT Type, MType, IType, RType, AType, ZType, Sign;
FROM M3IR IMPORT Name, Alignment, Label;
...
FROM M3IR IMPORT TypeUID;

TYPE cg_t = M3IR.T;
TYPE typeid_t = M3IR.TypeUID;

TYPE T <: Public;

TYPE Public = M3IR_AssertFalse.T OBJECT
...
TYPE op_t = OBJECT
    op: Op;
    METHODS
          replay(replay: Replay_t; cg: M3IR.T);
END;
