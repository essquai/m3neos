...
IMPORT IR, CallExpr, Expr, ExprRep, Procedure, Formal, Type, SubrangeType;
...
PROCEDURE Compile (ce: CallExpr.T) =
  BEGIN
    ...
    IR.Rotate (Word_types[rep].cg_type);
  END Compile;
...
PROCEDURE CompileL (ce: CallExpr.T) =
  BEGIN
    ...
    IR.Rotate_left (Word_types[rep].cg_type);
  END CompileL;
...
PROCEDURE CompileR (ce: CallExpr.T) =
  BEGIN
    ...
    CheckExpr.EmitChecks (ce.args[1], TInt.Zero, max,
                          IR.RuntimeError.ValueOutOfRange);
    IR.Rotate_right (Word_types[rep].cg_type);
  END CompileR;
