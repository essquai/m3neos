...
IMPORT IR, CallExpr, Expr, ExprRep, Procedure, Target, TWord;
...
PROCEDURE Compile (ce: CallExpr.T) =
  BEGIN
    ...
    IR.Compare (Word_types[rep].cg_type, IR.Cmp.GE);
  END Compile;
...
PROCEDURE PrepBR (ce: CallExpr.T;  true, false: IR.Label;  freq: IR.Frequency)=
  BEGIN
    ...
    Expr.Compile (ce.args[0]);
    Expr.Compile (ce.args[1]);
    IR.If_then (Word_types[rep].cg_type, IR.Cmp.GE, true, false, freq);
  END PrepBR;
