...
IMPORT M3, IR, Expr, ExprRep, Type, Int, LInt, Reel, LReel, EReel;
...
CONST
  IRType = ARRAY [cREAL..cADDR] OF IR.Type {
             IR.Type.Reel, IR.Type.LReel, IR.Type.XReel,  IR.Type.Addr };

TYPE
  Op = [ IR.Cmp.GT .. IR.Cmp.LE ];
...
PROCEDURE Check (p: P;  VAR cs: Expr.CheckState) =
  ...
  BEGIN
    ...
    IF (p.class = cSET)
      AND ((p.op = IR.Cmp.LT) OR (p.op = IR.Cmp.GT))
...
PROCEDURE Compile (p: P; <*UNUSED*> StaticOnly: BOOLEAN) =
  VAR type: IR.Type;  ta, tb, tmp: IR.Val;  info: Type.Info;
  BEGIN
    IF (p.class # cSET) THEN
      ...
      ELSE
        type := IRType [p.class];
      END;
      IR.Compare (type, p.op);

    ELSIF (p.bad_set) THEN
      Expr.Compile (p.a);  ta := IR.Pop ();
      Expr.Compile (p.b);  tb := IR.Pop ();
      IF (p.op = IR.Cmp.GT) THEN tmp := ta;  ta := tb;  tb := tmp END;
      IR.Push (ta);
      IR.Push (tb);
      IR.Compare (Target.Word.cg_type, IR.Cmp.NE);
      IR.Push (ta);
      IR.Push (tb);
      IR.Or (Target.Word.cg_type);
      IR.Push (tb);
      IR.Compare (Target.Word.cg_type, IR.Cmp.EQ);
...
PROCEDURE PrepBR (p: P;  true, false: IR.Label;  freq: IR.Frequency) =
  VAR type: IR.Type;  ta, tb, tmp: IR.Val;
      info: Type.Info;
  BEGIN
    Expr.Prep (p.a);
    Expr.Prep (p.b);
    IF (p.class # cSET) THEN
      ...
      ELSE
        type := IRType [p.class];
      END;
      IR.If_then (type, p.op, true, false, freq);
    ...
    ELSIF (p.bad_set) THEN
      ...
      IR.Push (ta);
      IR.Push (tb);
      IR.Not (Target.Word.cg_type);
      IR.And (Target.Word.cg_type);
      IR.Load_integer (Target.Word.cg_type, TInt.Zero);
      IR.Compare (Target.Word.cg_type, IR.Cmp.EQ);
      IR.And (Target.Word.cg_type);
      IF (true # IR.No_label)
