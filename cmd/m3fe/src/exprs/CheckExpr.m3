...
IMPORT M3, IR, Expr, ExprRep, Type, IntegerExpr, EnumExpr, Host;
...
PROCEDURE Compile (p: P; StaticOnly: BOOLEAN) =
  VAR t := Expr.TypeOf (p.expr);  cg_type: IR.Type;
  BEGIN
    ...
    CASE p.class OF
    | Class.cLOWER => IR.Check_lo (cg_type, p.min, p.err);
    | Class.cUPPER => IR.Check_hi (cg_type, p.max, p.err);
    | Class.cBOTH  => IR.Check_range (cg_type, p.min, p.max, p.err);
    END;
  END Compile;
...
(* Compiles 'e' and ensures that it's contained in [min..max].
  ...
  VAR minE, maxE: Target.Int;  x: Expr.T;
      t := Expr.TypeOf (e);  cg_type: IR.Type;
  BEGIN
    IF Type.IsSubtype (t, LInt.T)
      THEN cg_type := Target.Longint.cg_type;
      ELSE cg_type := Target.Integer.cg_type;
    ...
    IF Host.doRangeChk THEN
      ...
      IF TInt.LT (minE, min) AND TInt.LT (max, maxE) THEN
        IR.Check_range (cg_type, min, max, err);
