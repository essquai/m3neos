...
IMPORT IR, Expr, ExprRep, Type, Int, LInt, Reel, EReel, Target;
...
CONST
  IRType = ARRAY [cREAL..cEXTND] OF IR.Type {
...
PROCEDURE Compile (p: P; StaticOnly: BOOLEAN) =
  ...
  BEGIN
    ...
    ELSIF (p.class = cLINT) THEN
      Expr.Compile (p.a);
      Expr.Compile (p.b);
      IR.Multiply (Target.Longint.cg_type);
    ELSIF (p.class # cSET) THEN
      Expr.Compile (p.a);
      Expr.Compile (p.b);
      IR.Multiply (IRType [p.class]);
    ELSE
      EVAL Type.CheckInfo (p.type, info);
      size := info.size;
      IF (size > Target.Integer.size) THEN
