...
PROCEDURE PrepBR (p: P;  true, false: IR.Label;  freq: IR.Frequency) =
  ...
  BEGIN
    ...
    ELSE (* false = NoLabel => fall through *)
      skip := IR.Next_label ();
      Expr.PrepBranch (p.a, IR.No_label, skip, IR.Always - freq);
      Expr.CompileBranch (p.a, IR.No_label, skip, IR.Always - freq);
      Expr.PrepBranch (p.b, true, IR.No_label, freq);
      Expr.CompileBranch (p.b, true, IR.No_label, freq);
      IR.Set_label (skip);
    END;
  END PrepBR;
