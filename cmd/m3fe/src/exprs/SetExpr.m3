...
IMPORT M3, IR, Expr, ExprRep, Type, Error, IntegerExpr, EnumExpr;
...
PROCEDURE New (type: Type.T;  args: Expr.List): Expr.T =
  ...
  BEGIN
    ...
    p.is_const := TRUE;
    p.checked := FALSE;
    p.RTErrorCode := IR.RuntimeError.Unknown;
    p.RTErrorMsg := NIL;
    RETURN p;
  END New;
...
PROCEDURE MergeRTError (p: P; Code: IR.RuntimeError; Msg: TEXT) =
  BEGIN
...
(* NOTE^ If ever duplicate static sets are removed as for arrays, this
    ...
    ELSIF p.nOthers <= 0 THEN
      ...
      IR.Load_addr_of (Module.GlobalData (TRUE), p.globalOffset, info.alignment);
    ELSE
      (* Multi-word set with elements non-constant and/or outside host INTEGER
         range. We built it at Prep time. *)
      IR.Load_addr_of_temp (p.tmp, 0, Target.Integer.align);
      p.tmp := NIL;
    END;
  END Compile;

PROCEDURE PrepBig (p: P;  VAR info: Type.Info): IR.Var =
...
PROCEDURE EmitAssign (set: IR.Var;  wordNo: INTEGER;
                      READONLY value: Target.Int) =
  VAR tmp: Target.Int;
  BEGIN
    EVAL TInt.Extend (value, Target.Integer.bytes, tmp);
    IR.Load_integer (Target.Integer.cg_type, tmp);
    <* ASSERT Grain = Target.Integer.size *>
    IR.Store_int (Target.Integer.cg_type, set, wordNo * Grain);
  END EmitAssign;

PROCEDURE GenElement (e: Expr.T;  READONLY minT, maxT: Target.Int) =
  VAR cg_type := Type.IRType (Type.Base (Expr.TypeOf (e)));
  BEGIN
    CheckExpr.EmitChecks (e, minT, maxT, IR.RuntimeError.ValueOutOfRange);
    IF NOT TInt.EQ (minT, TInt.Zero) THEN
      IR.Load_integer (cg_type, minT);
      IR.Subtract (cg_type);
    END;
...
(* PRE: p.others contains no constants with values outside of host INTEGER. *)
  ...
  BEGIN
    ...
    (* push the mask *)
    IR.Load_integer (Target.Word.cg_type, curMask);
    IR.ForceStacked ();
 ...
 BEGIN
    ...
    IF info.size <= Target.Integer.size THEN
      ...
      WHILE (n # NIL) DO
        ...
        n := n.next;
      END (*WHILE*);
      IR.Init_int (offset, info.size, curMask, is_const);
      ...
      WHILE (n # NIL) DO
        ...
        IF (loWordNo # curWordNo) THEN
          ...
          IF NOT TInt.EQ (curMask, TInt.Zero) THEN
            ...
            IR.Init_int (offset + curWordNo*Target.Integer.pack,
        ...
        IF (loWordNo # hiWordNo) THEN
          ...
          IR.Init_int (offset + loWordNo * Target.Integer.pack, Target.Integer.size,
                       tmpT, is_const);
          FOR i := loWordNo + 1 TO hiWordNo - 1 DO
            EVAL TInt.Extend (full, Target.Integer.bytes, tmpT);
            IR.Init_int (offset + i * Target.Integer.pack, Target.Integer.size,
                         tmpT, is_const);
          END;
          curWordNo := hiWordNo;
          curMask := right [hiBitNo];
...
(* Return the first-discovered error found and stored during Check. *)
  ...
  BEGIN
    ...
    | P(p) =>
      <* ASSERT p.checked *>
      IF p.RTErrorCode # IR.RuntimeError.Unknown THEN
        Code := p.RTErrorCode;
        Msg := p.RTErrorMsg;
      END;
    END;
