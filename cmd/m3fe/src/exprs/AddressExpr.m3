...
IMPORT M3, IR, Expr, ExprRep, Type, Addr, Null, IntegerExpr;
...
PROCEDURE Compile (p: P; <*UNUSED*> StaticOnly: BOOLEAN) =
  ...
  BEGIN
    <*ASSERT b*>
    IR.Load_nil ();
    IF (val # 0) THEN IR.Add_offset (Target.Byte * val) END;
  END Compile;
...
PROCEDURE GenLiteral (p: P;  offset: INTEGER;  <*UNUSED*> type: Type.T;
  ...
  BEGIN
    IF NOT TInt.EQ (p.value, TInt.Zero) THEN
      IR.Init_int (offset, MIN (Target.Integer.size, Target.Address.size),
                   p.value, is_const);
    END;
  END GenLiteral;
