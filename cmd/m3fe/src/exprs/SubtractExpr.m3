...
IMPORT IR, Expr, ExprRep, Type, Error, LInt, Int, Reel, EnumType;
...
CONST
  FPType = ARRAY [Class.cREAL .. Class.cEXTND] OF IR.Type {
...
PROCEDURE Prep (p: P) =
  VAR info: Type.Info; m3t : IR.TypeUID;
  BEGIN
    Expr.Prep (p.a);
    Expr.Prep (p.b);
...
PROCEDURE Compile (p: P; StaticOnly: BOOLEAN) =
  ...
  BEGIN
    ...
    | Class.cLINT =>
        Expr.Compile (p.a);
        Expr.Compile (p.b);
        IR.Subtract (Target.Longint.cg_type);
    | Class.cREAL, Class.cLONG, Class.cEXTND =>
        Expr.Compile (p.a);
        Expr.Compile (p.b);
        IR.Subtract (FPType [p.class]);
    | Class.cSET  =>
        EVAL Type.CheckInfo (p.type, info);
        size := info.size;
    ...
    | Class.cADDR =>
        ...
        ELSE (* Addr - Addr *)
          ...
          Expr.Compile (p.b);
          IR.Loophole (IR.Type.Addr, Target.Word.cg_type);
          IR.Subtract (Target.Word.cg_type);
        END;
    END;
  END Compile;
