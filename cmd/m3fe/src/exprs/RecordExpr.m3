...
IMPORT M3, M3ID, IR, Error, Type, RecordType, Module;
...
PROCEDURE InnerPrepLV (p: P;  traced: BOOLEAN; usesAssignProtocol: BOOLEAN) =
  ...
  BEGIN
    ...
    FOR i := 0 TO LAST (p.map^) DO
      WITH z = p.map^[i] DO
        ...
        IF usesAssignProtocol THEN
          IR.Push (p.finalVal);
          IF (field.offset # 0) THEN  IR.Add_offset (field.offset);  END;
        ELSE
          IR.Load_addr_of (resultVar, field.offset, info.alignment);
        END;
    ...
    ELSE
      IR.Load_addr_of_temp (resultVar, 0, info.alignment);
      p.finalVal := IR.Pop ();
    END;
    p.prepped := TRUE;
  END InnerPrepLV;
