...
IMPORT M3, M3Buf, IR, Target, Type, Expr;
...
REVEAL
  ...
  METHODS
    ...
    (* ^Puts literal value in global area of current unit, if it's constant and
       structured.  Does not leave address on IR stack.  *)
    prep         ()                                := NoPrep;
    compile      (StaticOnly: BOOLEAN)             := NoCompile;
    prepLV       (traced: BOOLEAN)                 := NotLValue;
    compileLV    (traced, StaticOnly: BOOLEAN)     := NotLValueBool;
    prepBR       (true, false: IR.Label;  freq: IR.Frequency) := NotBoolean;
    compileBR    (true, false: IR.Label;  freq: IR.Frequency) := NotBoolean;
...
PROCEDURE NotBoolean     (e: M3.Expr; t,f: IR.Label; freq: IR.Frequency);
PROCEDURE PrepNoBranch   (e: M3.Expr; t,f: IR.Label; freq: IR.Frequency);
PROCEDURE NoBranch       (e: M3.Expr; t,f: IR.Label; freq: IR.Frequency);
