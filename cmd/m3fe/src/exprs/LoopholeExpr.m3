...
IMPORT M3Buf, IR, Expr, ExprRep, Type, Error, OpenArrayType;
...
PROCEDURE Prep (p: P) =
  ...
  BEGIN
    ...
    | Kind.V_to_A =>
        ...
        INC (p.tmp_cnt);
        Expr.Prep (e);
        t1 := IR.Declare_local (M3ID.NoID, sz, z_align, IR.Type.Struct,
                                Type.GlobalUID (u),
                                in_memory := TRUE, up_level := FALSE,
                                f := IR.Never);
        Expr.Compile (e);
        IR.Store (t1, 0, sz, z_align, u_info.stk_type);
        IR.Load_addr_of (t1, 0, z_align);
        p.tmp := BuildArray (p, sz);
    | Kind.D_to_S =>
        Expr.PrepLValue (e, traced := FALSE);
    | Kind.S_to_S =>
...
PROCEDURE PushTmp (p: P;  align: INTEGER) =
  BEGIN
    DEC (p.tmp_cnt);
    IF (p.tmp_cnt <= 0) THEN
      IR.Load_addr_of_temp (p.tmp, 0, align);
      p.tmp := NIL;
    ELSE
      IR.Load_addr_of (p.tmp, 0, align);
    END;
  END PushTmp;

PROCEDURE BuildArray (p: P;  src_size: INTEGER): IR.Var =
  (* PRE: Address of elements is on top of IR stack. *)
  VAR
    array : IR.Var;
  ...
  BEGIN
    ...
    IR.Load_intt (src_size DIV elt_info.size);
    IR.Store_int (Target.Integer.cg_type, array, M3RT.OA_size_0);
    RETURN array;
  END BuildArray;
...
PROCEDURE CompileLV (p: P; traced: BOOLEAN; <*UNUSED*> StaticOnly: BOOLEAN) =
  ...
  BEGIN
    ...
    | Kind.D_to_S =>
         ...
         *
         * ?ForceStacked() inhibits optimizations done by m3front/src/misc/IR.m3?
