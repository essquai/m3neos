...
IMPORT IR, Expr, ExprRep, Type, Int, LInt, IntegerExpr, Target;
...
CONST
  IRType = ARRAY [Class.cREAL .. Class.cEXTND] OF IR.Type {
             IR.Type.Reel, IR.Type.LReel, IR.Type.XReel};
  ...
  BEGIN
    ...
    IF (p.class = Class.cINT) OR (p.class = Class.cLINT) THEN
      ...
      ELSE
        ...
        Expr.Compile (e1);
        Expr.Compile (e2);
        IR.Mod (cg_type, Expr.GetSign (e1), Expr.GetSign (e2));
      END;
    ELSE
      (* floating point: x MOD y == x - y * FLOOR (x / y)  *)
      EVAL Type.CheckInfo (p.a.type, info);
      ...
      IR.Load (p.tmp1, 0, sz, align, info.addr_align, cg_type);
      IR.Load (p.tmp2, 0, sz, align, info.addr_align, cg_type);
      IR.Divide (cg_type);
      IR.Cvt_int (cg_type, Target.Integer.cg_type, IR.Cvt.Floor);
      IR.Cvt_float (Target.Integer.cg_type, cg_type);
      IR.Load (p.tmp2, 0, sz, align, info.addr_align, cg_type);
      IR.Multiply (cg_type);
      IR.Load (p.tmp1, 0, sz, align, info.addr_align, cg_type);
      IR.Swap ();
