...
IMPORT IR, Expr, ExprRep, Type, Int, LInt, Reel, LReel, EReel;
...
CONST
  FPType = ARRAY [Class.cREAL .. Class.cEXTND] OF IR.AType {
...
PROCEDURE Compile (p: P; StaticOnly: BOOLEAN) =
  ...
  BEGIN
    ...
    | Class.cSET =>
        EVAL Type.CheckInfo (p.type, info);
        IF (info.size > Target.Word.size) THEN
          IR.Load_addr_of_temp (p.tmp, 0, Target.Word.align);
          p.tmp := NIL;
        ELSE
          Expr.Compile (p.a);
          Expr.Compile (p.b);
    ...
    | Class.cLINT =>
        Expr.Compile (p.a);
        Expr.Compile (p.b);
        IR.Add (Target.Longint.cg_type);
    | Class.cREAL, Class.cLONG, Class.cEXTND =>
        Expr.Compile (p.a);
        Expr.Compile (p.b);
        IR.Add (FPType[p.class]);
    END;
  END Compile;
