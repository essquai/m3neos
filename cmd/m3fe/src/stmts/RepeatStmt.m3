...
IMPORT IR, Expr, Type, Bool, Error, Stmt, StmtRep;
  ...
  BEGIN
    Marker.PushExit (top+1);
      ...
      IF (Stmt.Outcome.FallThrough IN oc) THEN
        IR.Gen_location (p.e_origin);
        Expr.PrepBranch (p.expr, IR.No_label, top, IR.Likely);
        Expr.CompileBranch (p.expr, IR.No_label, top, IR.Likely);
      END;
      IR.Set_label (top+1);
    Marker.Pop ();
