...
IMPORT IR, Stmt, StmtRep, Expr, Type, Error, Module, Target, TInt;
...
PROCEDURE CheckStaticRTErrExec
  (tlhs: Type.T;  rhsExpr: Expr.T;  VAR cs: Stmt.CheckState;
   VAR(*OUT*) Code: IR.RuntimeError; VAR(*OUT*) Msg: TEXT; IsError := FALSE
  ) =
(* Like Check, but if a warning is produced for a runtime error that is
   ...
   Code # IR.RuntimeError.Unknown and a message text. in Msg.*)
...
PROCEDURE CheckOrdinal
  (tlhs: Type.T;  rhsExpr: Expr.T;
   VAR(*OUT*) Code: IR.RuntimeError; VAR(*OUT*) Msg: TEXT; IsError: BOOLEAN
  ) =
...
(* FIXME: This is a mess.  This is a true CT error--the first case of
      ...
      ELSE
        ...
        Code := IR.RuntimeError.ValueOutOfRange;
...
PROCEDURE CheckProcedure
  (proc: Expr.T; VAR(*OUT*) Code: IR.RuntimeError; VAR(*OUT*) Msg: TEXT; IsError: BOOLEAN) =
  VAR name: M3ID.T;
  VAR obj: Value.T;
      ...
      IF nested THEN
        ...
        ELSE 
          Error.WarnID
            (2, Value.CName (obj),
             "Nested procedure not assignable at runtime (2.3.1).");
          Code := IR.RuntimeError.NarrowFailed;
...
(* Before Prepping the rhsExpr, give it some info it could need to decide where
   to build its result and whether it needs a LHS expr on the IR stack. *)
  VAR baseExpr: Expr.T;
  BEGIN
...
PROCEDURE DoEmit
  ...
  BEGIN
    ...
    | Type.Class.Procedure =>
        ...
        IR.Store_indirect (lhsRepTypeInfo.stk_type, 0, lhsRepTypeInfo.size);
    | Type.Class.Record =>
        AssignRecord
...
PROCEDURE AssignOrdinal (tlhs: Type.T;  rhsExpr: Expr.T;
  ...
  BEGIN
    ...
    CheckExpr.EmitChecks (rhsExpr, min, max, IR.RuntimeError.ValueOutOfRange);
    (* ^Which does Compile(rhsExpr), thus pushing its IR.Val. *)
    IR.Store_indirect (lhsTypeInfo.stk_type, 0, lhsTypeInfo.size);
  END AssignOrdinal;
...
(* TODO: Merge AssignSet and AssignRecord.  Or maybe not. *)
    (* Leave the LHS address on the IR stack, regardless of protocol. *)
    IF Type.IsStructured (tlhs) THEN
      ...
      ELSE
        IR.Copy (lhsTypeInfo.size, overlap := NOT initializing);
      END
    ELSE (* Small set. *)
...
(* PRE: IRstack: RHS addr on top, LHS addr below. *)
...
PROCEDURE AssignRecord
  ...
  BEGIN
    ...
    THEN (* Compile will have copied RHS into LHS, leaving only the LHS
            address on IR stack, which is the expression's result. *) 
      IR.Discard (IR.Type.Addr);
    ELSE (* Using expression protocol. *)
      (* RHS is on top of IR stack.  LHS is below.
  ...
  BEGIN
    ...
    THEN
      ...
      (* CompileStruct will have done any needed shape check and copied
         rhsExpr's value into the LHS, leaving only the LHS address on IR
    ...
    ELSE (* Using expression protocol. *)
      ...
      THEN(* Both sides are fixed length arrays *)
        CompileStruct (rhsExpr);
        rhsAlign := Expr.Alignment (rhsExpr);
        (* RHS is on top of IR stack.  LHS is below, unmolested by Compile. *)
        CopyStruct
      ...
      ELSE (* Something is open. *)
        ...
        IF LHSIsOpen THEN
          IF RHSIsOpen THEN (* Both sides are open. *) 
            IR.Push (lhsVal);
            IR.Open_elt_ptr (lhsAlign);
            IR.ForceStacked ();
            IR.Push (rhsVal);
            IR.Open_elt_ptr (Expr.Alignment(rhsExpr));
            IR.ForceStacked ();
...
PROCEDURE GenOpenArrayRTShapeChecks       
  (lhsRepType: Type.T; lhsDopeVal: IR.Val; rhsRepType: Type.T; rhsVal: IR.Val) =
(* Leaves the IR stack as it is. *)
    ...
    WHILE ArrayType.Split (lhsRepType, lhsIndexType, lhsEltType)
      ...
      IF lhsIndexType = NIL THEN
        ...
        ELSE (* LHS is open and RHS is fixed. *)
          IR.Push (lhsDopeVal);
          IR.Open_size (depth);
          IR.Load_integer (Target.Integer.cg_type, Type.Number (rhsIndexType));
          IR.Check_eq
            (Target.Integer.cg_type, IR.RuntimeError.IncompatibleArrayShape);
        END
      ELSE
        IF rhsIndexType = NIL THEN (* LHS is fixed and RHS is open. *)
          IR.Push (rhsVal);
          IR.Open_size (depth);
          IR.Load_integer (Target.Integer.cg_type, Type.Number (lhsIndexType));
          IR.Check_eq
            (Target.Integer.cg_type, IR.RuntimeError.IncompatibleArrayShape);
...
PROCEDURE GenOpenArrayCopy (lhsDopeVal: IR.Val;  lhsRepType, rhsRepType: Type.T) =
...
*)
  ...
  BEGIN
    ...
    FOR i := 0 TO MIN (lhsDepth, rhsDepth) - 1 DO
      IR.Push (lhsDopeVal);
      IR.Open_size (i);
      IF (i # 0) THEN IR.Multiply (Target.Word.cg_type) END;
    END;
    IF lhsDepth < rhsDepth
    THEN IR.Copy_n (OpenArrayType.EltPack (lhsRepType), overlap := TRUE);
    ELSE IR.Copy_n (OpenArrayType.EltPack (rhsRepType), overlap := TRUE);
    END;
...
PROCEDURE AssignOrRTCheckProcedure (rhsExpr: Expr.T; lhsIsPushed: BOOLEAN) =
  ...
  BEGIN
    ...
    | RTCheckKind.Conditional =>
      ...
      rhsVal := IR.Pop ();
      ok := IR.Next_label ();
      IR.If_closure (rhsVal, IR.No_label, ok, IR.Always);
      IR.Abort (IR.RuntimeError.NarrowFailed);
...
PROCEDURE RTCheckArray (tlhs: Type.T;  rhsExpr: Expr.T) =
  ...
  BEGIN
    ...
    ELSIF RHSIsOpen THEN
      ...
      IR.Push (rhs);
      IR.Open_elt_ptr (ArrayType.EltAlign (trhs));
      IR.Free (rhs);
...
PROCEDURE GenOpenArraySizeChk (rhsVal: IR.Val;  tlhs, trhs: Type.T) =
  VAR lhsIndexType, rhsIndexType, lhsEltType, rhsEltType: Type.T;
  VAR depth := 0;
  BEGIN
    IF NOT Host.doNarrowChk THEN RETURN END;
...
(* EXPORTED: *) 
PROCEDURE DoGenRTAbort ( Code: IR.RuntimeError ): BOOLEAN =
  BEGIN
    CASE Code OF
    | IR.RuntimeError.ValueOutOfRange
    => RETURN Host.doRangeChk;
    | IR.RuntimeError.NarrowFailed
