...
IMPORT IR, Expr, Type, Bool, Error, Marker, ErrType;
...
PROCEDURE Compile (p: P): Stmt.Outcomes =
  VAR oc: Stmt.Outcomes;  top := IR.Next_label (3);
  BEGIN
    Marker.PushExit (top+2);
      ...
      IR.Set_label (top+1);
      Expr.PrepBranch (p.cond, top, IR.No_label, IR.Likely);
      Expr.CompileBranch (p.cond, top, IR.No_label, IR.Likely);
      IR.Set_label (top+2);
    Marker.Pop ();
