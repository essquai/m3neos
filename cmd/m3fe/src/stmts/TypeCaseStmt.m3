...
IMPORT M3ID, IR, Expr, Stmt, StmtRep, Type, Variable, Scope;
...
PROCEDURE Compile (p: P): Stmt.Outcomes =
  VAR
    ...
    l_base    := IR.Next_label (n_cases);
    l_else    := IR.Next_label ();
    l_exit    := IR.Next_label ();
    ref_type  := Type.Base (Expr.TypeOf (p.expr));
    labels    := NEW (REF ARRAY OF IR.Label, n_labels+1);
  ...
  BEGIN
    ...
    BEGIN
      ...
      WHILE (c # NIL) DO
        FOR z := 0 TO c.nTags - 1 DO
          ...
          IR.Init_intt (uid_offset, Target.Integer.size,
    ...
    Expr.Prep (p.expr);
    Expr.Compile (p.expr);
    ref := IR.Declare_local (M3ID.NoID, Target.Address.size,
    ...
    ELSIF (NOT foundForSure) AND (Host.doTCaseChk) THEN
      IR.Abort (IR.RuntimeError.UnhandledTypecase);
    END;
...
PROCEDURE CompileCaseBody (p: P;  c: Case;  ref: IR.Var;
                           case_label, exit_label: IR.Label;
                           VAR done: BOOLEAN): Stmt.Outcomes =
  VAR oc: Stmt.Outcomes;  zz: Scope.T;
  BEGIN
    IR.Gen_location (c.origin);
    IR.Set_label (case_label);
    ...
    IF (c.scope # NIL) THEN
      zz := Scope.Push (c.scope);
        ...
        IF (Stmt.Outcome.FallThrough IN oc) THEN IR.Jump (exit_label); END;
