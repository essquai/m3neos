...
IMPORT IR, Expr, Stmt, StmtRep, Type, Error, Target, TInt, Host;
...
PROCEDURE GenIndexedBranch (p: P;  l_min, l_max: INTEGER;
  ...
  VAR
    ...
    l_else, l_end, l_case, l_bodies: IR.Label;
    labels: REF ARRAY OF IR.Label;
    min, max, j: INTEGER;
  BEGIN
    Expr.GetBounds (p.expr, e_min, e_max);

    (* allocate the label's we need *)
    l_bodies := IR.Next_label (p.nCases);
    l_else := IR.Next_label ();
    labels := NEW (REF ARRAY OF IR.Label, l_max - l_min + 1);
    ...
    ELSIF TInt.LE (L_min, e_min) THEN
      ...
      x := IR.Pop ();
      IR.Push (x);
      IR.Load_intt (l_max - l_min);
      IR.If_compare (Target.Integer.cg_type, IR.Cmp.GT, l_else, IR.Never);
      IR.Push (x);
      IR.Free (x);
    ELSIF TInt.LE (e_max, L_max) THEN
      ...
      IR.If_compare (Target.Integer.cg_type, IR.Cmp.LT, l_else, IR.Never);
      IR.Push (x);
      IR.Free (x);
    ELSE
    ...
    FOR i := 0 TO p.nCases - 1 DO
      IR.Set_label (l_bodies + i);
      xc := Stmt.Compile (p.bodies[i]);
      oc := oc + xc;
      IF (Stmt.Outcome.FallThrough IN xc) THEN  IR.Jump (l_end)  END;
    END;
    ...
    ELSIF (NOT p.complete) AND (Host.doCaseChk) THEN
      IR.Abort (IR.RuntimeError.UnhandledCase);
    END;
  ...
  BEGIN
    ...
    WHILE (t # NIL) DO
      ...
      IF TInt.LT (next, t.min) THEN
        IR.Push (x);
        IR.Load_integer (Target.Integer.cg_type, t.min);
        IR.If_compare (Target.Integer.cg_type, IR.Cmp.LT, l_else, IR.Never);
      END;
      IR.Push (x);
      IR.Load_integer (Target.Integer.cg_type, t.max);
      IR.If_compare (Target.Integer.cg_type, IR.Cmp.LE, l_bodies+t.body, IR.Maybe);
      ...
      t := t.greater;
    END;
    IF TInt.LE (next, e_max) THEN  IR.Jump (l_else)  END;
    IR.Free (x);
