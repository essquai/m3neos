...
IMPORT IR, Expr, Token, Scanner, Stmt, StmtRep, Error, M3RT;
...
PROCEDURE Compile (p: P): Stmt.Outcomes =
  VAR v: Expr.T;  i: Target.Int;  u: Type.T;   skip: IR.Label;
  BEGIN
    IF Host.doDebugs THEN
      ...
      ELSIF (v = NIL) THEN
        ...
        skip := IR.Next_label ();
        PrepMsgs (p);
        Expr.PrepBranch (p.cond, IR.No_label, skip, IR.Always);
        Expr.CompileBranch (p.cond, IR.No_label, skip, IR.Always);
        EmitDebug (p);
        IR.Set_label (skip);
...
PROCEDURE EmitDebug (p: P) =
  ...
  BEGIN
    ...
    IR.Load_addr_of (msgs, M3RT.OA_size_1, Target.Address.align);
    IR.Store_addr (msgs, M3RT.OA_elt_ptr);

    (* initialize the count of array sizes *)
    IR.Load_intt (p.n_msgs);
    IR.Store_int (Target.Integer.cg_type, msgs, M3RT.OA_size_0);

    (* initialize each message *)
    offset := M3RT.OA_size_1;
    FOR i := 0 TO p.n_msgs-1 DO
      Expr.Compile (p.msgs[i]);
      IR.Store_addr (msgs, offset);
    ...
    IF Target.DefaultCall.args_left_to_right THEN
      (* module data pointer *)
        IR.Load_addr_of (Module.GlobalData (FALSE), 0, IR.Max_alignment);
        IR.Pop_param (IR.Type.Addr);
      (* line number *)
        IR.Load_intt (this_line);
        IR.Pop_param (Target.Integer.cg_type);
      (* messages *)
        IR.Load_addr_of (msgs, 0, Target.Address.align);
        IR.Pop_param (IR.Type.Addr);
    ELSE
      (* messages *)
        IR.Load_addr_of (msgs, 0, Target.Address.align);
