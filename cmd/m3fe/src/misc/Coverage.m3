...
IMPORT Text, M3ID, Value, Host, Scanner, Target, TInt, IR;
...
PROCEDURE GenerateTables () =
  ...
  BEGIN
    ...
    tbl := IR.Declare_global (M3ID.NoID, size, Target.Address.align,
                              IR.Type.Struct, 0, exported := FALSE, init := TRUE);

    (* initialize the coverage tables *)
    IR.Begin_init (tbl);
    size := 0;

    IR.Init_chars (size, Header, FALSE);
    INC (size, l_header * Target.Char.size);     (*header*)

    (* IR.Init_int (size, Target.Integer.size, TInt.Zero, FALSE); *)
    INC (size, Target.Integer.size);             (*timestamp*)

    IR.Init_intt (size, Target.Integer.size, Text.Length (Host.filename), FALSE);
    INC (size, Target.Integer.size);             (*fileLen*)

    IR.Init_chars (size, Host.filename, FALSE);
    INC (size, l_fname * Target.Char.size);      (*file*)

    IR.Init_intt (size, Target.Integer.size, minLine, FALSE);
    INC (size, Target.Integer.size);             (*firstLine*)

    IR.Init_intt (size, Target.Integer.size, nLines, FALSE);
    INC (size, Target.Integer.size);             (*nLines*)
    ...
    FOR x := 0 TO nLines-1 DO
      IF (used # NIL) AND (used [x+minLine] # LineSeen.no)
        ...
        ELSE len := -1;
      END;
      IR.Init_intt (size, Target.Integer.size, len, FALSE);
      INC (size, Target.Integer.size);    (*lines[x]*)
    END;

    IR.Init_intt (size, Target.Integer.size, i, FALSE);
    INC (size, Target.Integer.size);             (*nProcs*)
    ...
    WHILE (p # NIL) DO
      IF (p.proc # NIL) THEN
        ...
        IR.Init_intt (size, Target.Integer.size, Text.Length (p.name), FALSE);
        INC (size, Target.Integer.size);         (*len[p]*)

        IR.Init_chars (size, p.name, FALSE);
        INC (size, len * Target.Char.size);      (*pname[p]*)

        (* IR.Init_int (size, Target.Integer.size, 0, FALSE); *)
        p.offset := size;
        INC (size, Target.Integer.size);         (*cnt[p]*)
      END;
      p := p.next;
    ...
    IR.Init_chars (size, Trailer, FALSE);
    INC (size, l_trailer * Target.Char.size);    (*trailer*)
...
PROCEDURE CountLine () =
  ...
  BEGIN
    ...
    IR.Load_int (Target.Integer.cg_type, tbl, offset);
    IR.Load_integer (Target.Integer.cg_type, TInt.One);
    IR.Add (Target.Word.cg_type);
    IR.Store_int (Target.Integer.cg_type, tbl, offset);
...
PROCEDURE CountProcedure (v: Value.T) =
  ...
  BEGIN
    ...
    IR.Load_int (Target.Integer.cg_type, tbl, p.offset);
    IR.Load_integer (Target.Integer.cg_type, TInt.One);
    IR.Add (Target.Word.cg_type);
    IR.Store_int (Target.Integer.cg_type, tbl, p.offset);
  END CountProcedure;
