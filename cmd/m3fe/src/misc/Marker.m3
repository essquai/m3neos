...
IMPORT IR, Error, Type, Variable, ProcType, ESet, Expr, AssignStmt;
...
PROCEDURE Push (k: Kind;  l_start, l_stop: IR.Label := IR.No_label;
                info: IR.Var := NIL;  ex: ESet.T := NIL) =
  BEGIN
    WITH z = stack[tos] DO
      z.kind       := k;
      z.saved      := FALSE;
...
PROCEDURE PushFrame (frame: IR.Var;  class: M3RT.HandlerClass) =
  ...
  BEGIN
    IR.Load_intt (ORD (class));
    IR.Store_int (Target.Integer.cg_type, frame, M3RT.EF_class);
    push := RunTyme.LookUpProc (RunTyme.Hook.PushEFrame);
    Procedure.StartCall (push);
    IR.Load_addr_of (frame, 0, Target.Address.align);
...
PROCEDURE CaptureState (frame: IR.Var;  jmpbuf: IR.Var;  handler: IR.Label) =
  ...
  BEGIN
    ...
    IR.Start_call_direct (setjmp, 0, Target.Integer.cg_type);
    ...
    ELSE
       ...
       *)
      IR.Load_addr_of (frame, M3RT.EF1_jmpbuf, jmpbufAlign);
    END;
    IR.Pop_param (IR.Type.Addr);
    ...
    IF Target.Sigsetjmp THEN
      IR.Load_intt (0);             (* do not save signal mask *)
  ...
  BEGIN
    ...
    WHILE (i >= 0) DO
      WITH z = stack[i] DO
        CASE z.kind OF
        | Kind.zTRYELSE =>
            IR.Load_intt (Exit_exception);
            IR.Store_int (Target.Integer.cg_type, z.info);
            IR.Jump (z.stopBody);
            EXIT;
      ...
      WHILE (i >= 0) DO
        WITH z = stack[i] DO
          ...
          | Kind.zTRYELSE =>
              Expr.Prep (expr);
              Expr.Compile (expr);
              IR.Discard (Type.IRType (Expr.TypeOf (expr)));
              expr := NIL;
              EXIT;
      ...
      IF (expr # NIL) THEN
        WITH z = stack[i] DO
          ...
          IF Host.direct_struct_assign AND is_large AND NOT simple THEN
            ...
            IR.Load_addr_of (z.tmp_result, 0, ret_info.alignment);
    ...
    IF i >= 0 THEN
      WITH z = stack[i] DO
        ...
        THEN
          ...
          EVAL Type.CheckInfo (z.type, ret_info);
          Variable.LoadLValue (z.variable);
          IR.Load_addr_of_temp (z.tmp_result, 0, ret_info.alignment);
          IR.Copy (ret_info.size, overlap := FALSE);
        END;
        IF (z.type = NIL) THEN
        ...
        ELSIF fromFinally THEN
          ...
          IF NOT ProcType.LargeResult (z.type) THEN
            ...
            IR.Exit_proc (Type.IRType (z.type));
          ELSIF (z.callConv.standard_structs) THEN
            IR.Exit_proc (IR.Type.Void);
          ELSE
            Variable.LoadLValue (z.variable);
            IR.Exit_proc (IR.Type.Struct);
          END;
        ELSIF is_large THEN
  ...
  BEGIN
    ...
    WHILE (i >= 0) DO
      WITH z = stack[i] DO
        ...
        | Kind.zTRYELSE =>
            PopFrame (z.info);
            IR.Load_nil ();  (* the current "RETURN" exception is lost *)
            IR.Store_addr (z.info, M3RT.EF1_info + M3RT.EA_exception);
            IR.Jump (z.stop);
            EXIT;
        | Kind.zFINALLY =>
            PopFrame (z.info);
...
PROCEDURE EmitScopeTable (doEmit : BOOLEAN := FALSE): INTEGER =
  ...
  BEGIN
    ...
    IR.Comment (base, TRUE, "exception scopes");

    (* fill in the table *)
    f := all_frames;
    x := base;
    WHILE (f # NIL) DO
      IR.Init_intt  (x + M3RT.EX_class, Target.Char.size, RT_Kind [f.kind], TRUE);
      IF (f.outermost) THEN
        IR.Init_intt  (x + M3RT.EX_outermost, Target.Char.size, ORD(TRUE), TRUE);
      END;
      IF (f.next = NIL) THEN
        IR.Init_intt  (x + M3RT.EX_end_of_list, Target.Char.size, ORD(TRUE), TRUE);
      END;
      IR.Init_label (x + M3RT.EX_start, f.start, TRUE);
      IR.Init_label (x + M3RT.EX_stop, f.stop, TRUE);
      IF (f.info # NIL) THEN IR.Init_offset (x + M3RT.EX_offset, f.info, TRUE) END;
      IF (f.e_set # NIL) THEN
        ESet.GetAddress (f.e_set, e_base, e_offset);
        IF (e_base # NIL) OR (e_offset # 0) THEN
          IR.Init_var (x + M3RT.EX_excepts, e_base, e_offset, TRUE);
        END;
      END;
      INC (x, M3RT.EX_SIZE);
      f := f.next;
...
PROCEDURE EmitExceptionTest (signature: Type.T;  need_value: BOOLEAN): IR.Val =
  ...
  BEGIN
    ...
    ***)
    ...
    IR.Load_addr (stack[i].info, M3RT.EA_exception, Target.Address.align);
    IR.Load_nil ();
    IR.If_compare (IR.Type.Addr, IR.Cmp.NE, stack[i].stop, IR.Never);
    *)
...
PROCEDURE CaptureResult (result: IR.Type): IR.Val =
  BEGIN
    IF (result = IR.Type.Void) THEN
      RETURN NIL;
    ELSIF (result # IR.Type.Struct) THEN
      RETURN IR.Pop ();
    ELSE
      IR.Discard (result);
      RETURN NIL;
