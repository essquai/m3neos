...
IMPORT IR, Module, Target, M3ID, Fmt, Error;
...
PROCEDURE CompileTryGetJmpbuf (VAR try: Try): IR.Var =
...
PROCEDURE CompileProcAllocateJmpbufs (p: Proc) =
  ...
  BEGIN
    ...
    <* ASSERT try_count >= 0 *>
    IR.Comment(-1, FALSE, "AllocateJmpbufs " & IdToText(p.name)
    ...
    p.jmpbufs := NEW(REF ARRAY OF IR.Var, try_count);
    FOR i := 0 TO try_count - 1 DO
      p.jmpbufs[i] := IR.Declare_local(M3ID.NoID, Target.Address.size,
                                       Target.Address.align, IR.Type.Addr,
                                       0, in_memory := FALSE,
                                       up_level := FALSE, f := IR.Likely);
    END;
    ...
    IR.Start_call_direct(alloca, 0, Target.Address.cg_type);
    IR.Load_int(Target.Word.cg_type, size);
    IF try_count # 1 THEN
      IR.Load_intt(try_count);
    ...
    IF try_count > 1 THEN
      FOR i := 1 TO try_count - 1 DO
        ...
        IR.Loophole(Target.Word.cg_type, Target.Address.cg_type);
        IR.Store_addr(p.jmpbufs[i]);
      END;
    END;
