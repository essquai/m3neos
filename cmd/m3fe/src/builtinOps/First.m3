...
IMPORT IR, CallExpr, Expr, ExprRep, Type, Procedure, Error, ArrayType;
  ...
  BEGIN
    ...
    ELSIF Type.GetBounds (t, min, max) THEN (* ordinal type *)
      IF Type.IsSubtype (t, LInt.T)
        THEN IR.Load_integer (Target.Longint.cg_type, min);
        ELSE IR.Load_integer (Target.Integer.cg_type, min);
      END;
    ELSIF Type.IsEqual (t, Reel.T, NIL) THEN
      IR.Load_float (Target.Real.min);
    ELSIF Type.IsEqual (t, LReel.T, NIL) THEN
      IR.Load_float (Target.Longreal.min);
    ELSIF Type.IsEqual (t, EReel.T, NIL) THEN
      IR.Load_float (Target.Extended.min);
    ELSE
      <* ASSERT FALSE *>
    END;
  END Compile;
