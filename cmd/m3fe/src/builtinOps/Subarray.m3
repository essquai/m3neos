...
IMPORT IR, CallExpr, Expr, ExprRep, Type, Procedure, Error, ArrayType, Card;
...
(* TODO: Warn on statically-detectable negative value. *)
    ...
    IF TInt.LT (min, TInt.Zero) OR TInt.LT (max, min) THEN
      e := CheckExpr.NewLower (e, TInt.Zero, IR.RuntimeError.ValueOutOfRange);
      Expr.TypeCheck (e, cs);
    END;
    RETURN e;
  END RTCheckNonNeg;
...
PROCEDURE PrepLV (ce: CallExpr.T; traced: BOOLEAN) =
  ...
  BEGIN
    ...
    | 2_000 =>  (* fixed array, var start, var len ------------------------------*)
          ...
          Expr.Compile (len);
          IR.Store_int (Target.Integer.cg_type, t_result, M3RT.OA_size_0);

          IF NOT Host.doRangeChk THEN
            Expr.CompileAddress (base, traced);
            Expr.Compile (start);
          ELSE
            Expr.Compile (start);         t_start := IR.Pop ();
            IR.Push (t_start);
            IR.Load_int (Target.Integer.cg_type, t_result, M3RT.OA_size_0);
            IR.Add (Target.Integer.cg_type);
            IR.Check_hi (Target.Integer.cg_type, n_elts,
          ...
          IR.Store_addr (t_result, M3RT.OA_elt_ptr);

    | 2_001 =>  (* fixed array, var start, const len ----------------------------*)

          (* initialize the new count *)
          IR.Load_integer (Target.Integer.cg_type, x_len);
          IR.Store_int (Target.Integer.cg_type, t_result, M3RT.OA_size_0);
          ...
          ELSIF NOT TInt.Subtract (n_elts, x_len, max) THEN
           ...
           Expr.Compile (start);         t_start := IR.Pop ();
            ...
            IR.Load_integer (Target.Integer.cg_type, x_len);
            IR.Add (Target.Integer.cg_type);
            IR.Check_hi (Target.Integer.cg_type,
                         n_elts, IR.RuntimeError.ValueOutOfRange);
           IR.Discard (Target.Integer.cg_type);
            Expr.CompileAddress (base, traced);
            IR.Push (t_start);
            IR.Free (t_start);
          ELSIF TInt.LT (max, TInt.Zero) THEN
            ...
            Expr.Compile (start);
            IR.Check_hi (Target.Integer.cg_type,
                         max, IR.RuntimeError.ValueOutOfRange);
          ELSE
            Expr.CompileAddress (base, traced);
            Expr.Compile (start);
    ...
    | 2_010 =>  (* fixed array, const start, var len ----------------------------*)
          ...
          ELSIF NOT TInt.Subtract (n_elts, x_start, max) THEN
            ...
            IR.Load_int (Target.Integer.cg_type, t_result, M3RT.OA_size_0);
            IR.Load_integer (Target.Integer.cg_type, x_start);
            IR.Add (Target.Integer.cg_type);
            IR.Check_hi (Target.Integer.cg_type, n_elts,
                         IR.RuntimeError.ValueOutOfRange);
            IR.Discard (Target.Integer.cg_type);
          ...
          (* initialize the new data pointer *)
          Expr.CompileAddress (base, traced);
          IR.Add_offset (i_start * elt_pack);
          IR.Store_addr (t_result, M3RT.OA_elt_ptr);
    ...
    | 2_011 =>  (* fixed array, const start, const len --------------------------*)
          ...
          ELSIF TInt.LT (n_elts, max) THEN
            ...
            IR.Load_integer (Target.Integer.cg_type, max);
            IR.Check_hi (Target.Integer.cg_type, n_elts,
                         IR.RuntimeError.ValueOutOfRange);
            IR.Discard (Target.Integer.cg_type);
          ELSE
    ...
    | 2_100 =>  (* open array, var start, var len -------------------------------*)
          ...
          Expr.CompileAddress (base, traced);   t_base  := IR.Pop ();
          Expr.Compile (start);                 t_start := IR.Pop ();
          ...
          (* initialize the new data pointer *)
          IR.Push (t_base);
          IR.Open_elt_ptr (ArrayType.EltAlign (array));
          IR.Push (t_start);
    ...
    IR.Load_addr_of_temp (t_result, 0, Target.Address.align);
    ce.tmp := IR.Pop ();
...
PROCEDURE CopyDopeVector (src: IR.Val;  dest: IR.Var;  depth: INTEGER) =
  BEGIN
...
PROCEDURE ComputeOffset (array: IR.Var;  depth, elt_pack: INTEGER) =
  BEGIN
    FOR i := 1 TO depth - 1 DO
    ...
    END;
    IR.Index_bytes (elt_pack);
    IR.Store_addr (array, M3RT.OA_elt_ptr);
  END ComputeOffset;
