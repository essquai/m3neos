...
IMPORT IR, CallExpr, Expr, ExprRep, Type, Procedure, IntegerExpr;
...
PROCEDURE DoCompile (e: Expr.T;  unit: INTEGER) =
  VAR t: Type.T;  sz, align: INTEGER;  t_array: IR.Val;  info: Type.Info;
  BEGIN
    IF TypeExpr.Split (e, t) THEN
      Type.Compile (t);
      EVAL Type.CheckInfo (t, info);
      <* ASSERT info.size > 0 *>
      IR.Load_intt ((info.size + unit - 1) DIV unit);
      RETURN;
    END;
