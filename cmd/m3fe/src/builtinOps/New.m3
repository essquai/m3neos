...
IMPORT IR, CallExpr, Expr, ExprRep, Type, Procedure, Error;
...
(*TODO:                       ^RefType.InitTypecell and possibly other places
  ...
  BEGIN
    ...
    IR.Load_addr_of (sizes, M3RT.OA_size_1, Target.Integer.align);
    ...
    FOR i := 1 TO n DO
      ...
      IR.Store_int (Target.Integer.cg_type, sizes, offset);
    ...
    IF Target.DefaultCall.args_left_to_right THEN
      Type.LoadInfo (t, -1);
      IR.Pop_param (IR.Type.Addr);
      IR.Load_addr_of (sizes, 0, sizesAlign);
      IR.Pop_param (IR.Type.Addr);
    ELSE
      IR.Load_addr_of (sizes, 0, sizesAlign);
      IR.Pop_param (IR.Type.Addr);
  ...
  BEGIN
    ...
    FOR i := 1 TO LAST (ce.args^) DO
      IF (ce.args[i] # NIL) THEN
        ...
        END;
        IR.Add_offset (field.offset);
        IR.Boost_addr_alignment (obj_align);
        AssignStmt.DoEmit (field.type, value, initializing := TRUE);
      END;
    END;
  END GenObject;
