...
IMPORT M3, M3ID, IR, Value, ValueRep, Type, Error, Expr, ProcType;
...
TYPE
  T = Value.T BRANDED OBJECT 
        ...
        tempCGVal: IR.Val     := NIL;
        cg_type  : IR.TypeUID := 0;
  ...
  BEGIN
    ...
    IF (types_only) THEN
      IF indirect THEN (* lo-level pass by reference. *)
        IR.Declare_typename (t.cg_type, typename);
        t.cg_type := IR.Declare_indirect (t.cg_type, typename);
      END;
    ELSIF (param) THEN
      trace := trace * 10 + 2;
      IF indirect THEN (* lo-level pass by reference. *)
      ...
      EVAL IR.Declare_param (t.name, size, align, mtype,
                             t.cg_type, in_memory := FALSE, up_level := FALSE,
                             f := IR.Maybe, typename := typename);
    ELSE (* This is part of debug info for a signature. *)
      ...
      IR.Declare_formal (t.name, t.cg_type, typename);
    END;
  END EmitDeclaration;
...
(* PRE: A scalar of type 'type' is on top of IR stack. It is not bitpacked. *)
    ...
    ***)
    ...
    ELSE
      ...
      | Mode.mVALUE =>
          ...
          CheckExpr.EmitChecks (actExpr, min, max, IR.RuntimeError.ValueOutOfRange);
          IR.Pop_param (Type.IRType (formVal.type, in_memory := TRUE));
      | Mode.mVAR =>
      ...
      | Mode.mREADONLY =>
          ...
          THEN (* Pass by value. Could need bounds check. *)
            ...
            CheckExpr.EmitChecks
              (actExpr, min, max, IR.RuntimeError.ValueOutOfRange);
...
PROCEDURE GenClosure (actExpr: Expr.T;  proc: Expr.T) =
  VAR tmp: IR.Var; proc_v: Value.T; n_elts: INTEGER;
  ...
  BEGIN
    IF RequiresClosure (actExpr, proc_v) THEN
      ...
      IR.Load_intt  (M3RT.CL_marker_value);
      IR.Store_int (Target.Integer.cg_type, tmp, M3RT.CL_marker);
      Procedure.LoadStaticLink (proc_v);
      IR.Store_addr (tmp, M3RT.CL_frame);
      IR.Load_addr_of (tmp,  0, Target.Address.align);
    END;
  END GenClosure;
...
PROCEDURE CompileNCopyStructWInWord
  ...
  (* Compile and copy a struct that lies within a word, leaving the address
     of the copy on top of the IR stack.  The original struct may be bitpacked.
     The copy will be word-aligned. *)
  VAR actTempVar: IR.Var;
  BEGIN
    ...
    THEN
      IR.Load_intt (Target.Word.size - actSize);
      IR.Shift_left (Target.Word.cg_type);
    END;
    IR.Load_addr_of (actTempVar, 0, Target.Word.align);
          ...
          ELSE (* Pass by value (2.3.2).  Lo-level, pass a copy by reference. *)
            ...
            ELSE (* Actual is byte-sized and aligned.  Pass by value (2.3.2).
              ...
              THEN (* Formal gives static size for the copy.
                ...
                Expr.Compile (actExpr);
                RedepthArray (formVal.repType, actRepType, 0);
                IR.Copy (formRepTypeInfo.size, overlap := FALSE);
                IR.Load_addr_of (actTempVar, 0, formRepTypeInfo.alignment);
                IR.Pop_param (IR.Type.Addr);
...
(* PRE: Address of actual is pushed on IR stack. *)
(* Change TOS array from actType's open depth to formType's open depth, while
  ...
  VAR dopeTempVar, eltsTempVar: IR.Var := NIL;
  VAR actVal: IR.Val := NIL;
  ...
  BEGIN
    ...
    IF eltsCopySize > 0 THEN (* Make a copy of elements. *)
      ...
      IR.Copy (eltsCopySize, overlap := FALSE);
    END;
    ...
    IF eltsCopySize > 0 OR formDepth > actDepth THEN (* New dope needed. *)
      ...
      IF eltsTempVar = NIL THEN
        IF actVal = NIL THEN actVal := IR.Pop () END;
        IR.Push (actVal);
        IF actDepth > 0 THEN IR.Open_elt_ptr (actTypeInfo.alignment) END;
      ELSE
        IR.Load_addr_of (eltsTempVar, M3RT.OA_elt_ptr, eltsAlign);
      END;
      IR.Store_addr (dopeTempVar, M3RT.OA_elt_ptr);
    ...
    IF formDepth > actDepth THEN
      ...
      FOR i := actDepth TO formDepth-1 DO
        ...
        <* ASSERT b *>
        <* ASSERT indexType # NIL *>
        IR.Load_integer (Target.Integer.cg_type, Type.Number (indexType));
        IR.Store_int (Target.Integer.cg_type,
    ...
    ELSE (* formDepth <= actDepth *)
      ...
      FOR i := formDepth TO actDepth - 1 DO
        ...
        IR.Open_size (i);
        IR.Load_integer (Target.Integer.cg_type, Type.Number (indexType));
        IR.Check_eq (Target.Integer.cg_type,
...
PROCEDURE GenCopy (type: Type.T) =
  ...
  VAR info: Type.Info;  copyVar: IR.Var;  id: IR.TypeUID;
  BEGIN
    EVAL Type.CheckInfo (type, info);
    id := Type.GlobalUID (type);
    IF Type.IsStructured (type) THEN
      copyVar := IR.Declare_local (M3ID.NoID, info.size, info.alignment,
                               IR.Type.Struct, id, in_memory := TRUE,
                               up_level := FALSE, f := IR.Never);
      IR.Load_addr_of (copyVar, 0, info.alignment);
      IR.Swap ();
    ...
    ELSE
      copyVar := IR.Declare_local (M3ID.NoID, info.size, info.alignment,
                               ...
                               up_level := FALSE, f := IR.Never);
      IR.Store (copyVar, 0, info.size, info.alignment, info.stk_type);
    END;
    IR.Load_addr_of (copyVar, 0, info.alignment);
  END GenCopy;
