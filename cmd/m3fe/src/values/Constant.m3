    ...
    ELSE
      ...
      IR.Declare_global_field
        (t.name, t.offset, Target.Address.size,
         IR.Declare_indirect (typeUID), is_const := FALSE);
...
PROCEDURE ConstInit (t: T) =
  BEGIN
    IF t.gen_init THEN
      t.gen_init := FALSE;
      IR.Comment (t.offset, FALSE, "Address of constant ", t.qualName);
      IR.Init_var (t.offset, Module.GlobalData (TRUE), t.coffset, FALSE);

      IR.Comment (t.coffset, TRUE, "Contents of constant ", t.qualName);
