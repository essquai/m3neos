...
IMPORT M3, M3ID, IR, Value, ValueRep, Scope, Stmt, Error, ESet,  External;
...
CONST
  ModuleTypeUID = -1;  (* special IR.TypeUID used for all interface records *)
...
PROCEDURE GetAlloca (t: T) : IR.Proc =
VAR new := FALSE;
BEGIN
...
PROCEDURE GetJmpbufSize (t: T): IR.Var =
BEGIN
  ...
  *)
  IF t.jmpbuf_size = NIL THEN
    t.jmpbuf_size := IR.Import_global (M3ID.Add ("m3_jmpbuf_size"),
...
PROCEDURE GetSetjmp (t: T): IR.Proc =
VAR new := FALSE;
BEGIN
  (*    int setjmp(void* );
  ...
  IF t.setjmp = NIL THEN
    ...
    IF new THEN
      ...
      IF Target.Sigsetjmp THEN
        EVAL IR.Declare_param (M3ID.Add ("save_mask"), Target.Int32.size, (* int *)
                               Target.Int32.align, IR.Type.Int32, 0,
                               in_memory := FALSE, up_level := FALSE,
                               f := IR.Never);
      END;
    END;
...
PROCEDURE Compile (t: T) =
  ...
  BEGIN
    ...
    zz := Scope.Push (t.localScope);
      IR.Begin_unit ();
      IR.Widechar_size (Target.WideCharSize());
      IR.Gen_location (t.origin);
      ...
      END;
      IF (load_map[FALSE] # NIL) THEN
        IR.Comment (-1, FALSE, "load map", Wr.EOL,
...
PROCEDURE DeclareGlobalData (t: T) =
  BEGIN
    IR.Comment (-1, FALSE, "module global constants");
    t.globals[TRUE].seg := IR.Declare_segment (M3ID.NoID,
                                                ModuleTypeUID, is_const := TRUE);
    IR.Comment (-1, FALSE, "module global data");
    t.globals[FALSE].seg := IR.Declare_segment (M3ID.Add (DataName (t)),
                                                ModuleTypeUID, is_const := FALSE);
  END DeclareGlobalData;

PROCEDURE GlobalData (is_const: BOOLEAN): IR.Var =
  BEGIN
...
PROCEDURE BuildImportLink (nm: M3ID.T;  binder: TEXT): INTEGER =
  ...
  BEGIN
    ...
    IR.Init_var (prev_link, curModule.globals[FALSE].seg, offset, FALSE);
    IR.Init_proc (offset + M3RT.II_binder, proc, FALSE);
    RETURN offset;
  END BuildImportLink;
...
PROCEDURE EmitBody (x: InitBody) =
  VAR t := x.self;  zz: Scope.T;   skip := IR.Next_label ();
  BEGIN
    IF (x.cg_proc = NIL) THEN RETURN END;
    ...
    IR.Comment (-1, FALSE, "module main body ", x.name);
    Scanner.offset := t.body_origin;
    IR.Gen_location (t.body_origin);
    IR.Begin_procedure (x.cg_proc);

    IR.Load_int (Target.Integer.cg_type, x.arg);
    IR.If_false (skip, IR.Never);
    ...
    IR.Load_addr_of (t.globals[FALSE].seg, 0, IR.Max_alignment);
    IR.Exit_proc (IR.Type.Addr);
...
PROCEDURE GenLinkerInfo (t: T;  proc_info, type_map, rev_full, rev_part: INTEGER) =
  ...
  BEGIN
    ...
    offs := IR.EmitText (file, is_const := TRUE);
    IR.Init_var (M3RT.MI_file, vc, offs, is_const := FALSE);
    IR.Comment (offs, TRUE, "file name");
    ...
    IF (type_cells >= 0) THEN
      IR.Init_var (M3RT.MI_type_cells, v, type_cells, FALSE);
    END;
    IF (type_cell_ptrs >= 0) THEN
      IR.Init_var (M3RT.MI_type_cell_ptrs, v, type_cell_ptrs, FALSE);
    END;
    IF (rev_full >= 0) THEN
      IR.Init_var (M3RT.MI_full_rev, vc, rev_full, FALSE);
    END;
    IF (rev_part >= 0) THEN
      IR.Init_var (M3RT.MI_part_rev, vc, rev_part, FALSE);
    END;
    IF (proc_info >= 0) THEN
      IR.Init_var (M3RT.MI_proc_info, vc, proc_info, FALSE);
    END;
    IF (exception_scopes >= 0) THEN
      IR.Init_var (M3RT.MI_try_scopes, vc, exception_scopes, FALSE);
    END;
    IF (type_map >= 0) THEN
      IR.Init_var (M3RT.MI_var_map, vc, type_map, FALSE);
      IR.Init_var (M3RT.MI_gc_map, vc, type_map, FALSE);
    END;
    IF (t.body # NIL) AND (t.body.cg_proc # NIL) THEN
      IR.Init_proc (M3RT.MI_binder, t.body.cg_proc, FALSE);
    END;
    IF (Host.doIncGC) AND (Host.doGenGC) THEN
      IR.Init_intt (M3RT.MI_gc_flags, Target.Integer.size, 3, FALSE);
    ELSIF (Host.doIncGC) THEN
      IR.Init_intt (M3RT.MI_gc_flags, Target.Integer.size, 2, FALSE);
    ELSIF (Host.doGenGC) THEN
      IR.Init_intt (M3RT.MI_gc_flags, Target.Integer.size, 1, FALSE);
    END;
    ...
    IR.Comment (-1, FALSE, "global constant type descriptor");
    IR.Emit_global_record (t.globals[TRUE].size, TRUE);
    IR.Comment (-1, FALSE, "global data type descriptor");
    IR.Emit_global_record (t.globals[FALSE].size, FALSE);

    (* finish the global data initializations *)
    IR.Comment (-1, TRUE, "module global constants");
    IR.Bind_segment (t.globals[TRUE].seg, t.globals[TRUE].size, IR.Max_alignment,
                     IR.Type.Struct, exported := FALSE, init := TRUE,
                     is_const := TRUE);
    IR.Comment (-1, FALSE, "module global data");
    IR.Bind_segment (t.globals[FALSE].seg, t.globals[FALSE].size, IR.Max_alignment,
                     IR.Type.Struct, exported := FALSE, init := TRUE,
                     is_const := FALSE);
  END GenLinkerInfo;
...
PROCEDURE BinderDeclareParam (): IR.Var =
BEGIN
  RETURN IR.Declare_param (M3ID.Add ("mode"), Target.Integer.size,
                           Target.Integer.align, Target.Integer.cg_type,
                           Type.GlobalUID (Int.T), (*in_memory*) FALSE,
                           (*up_level*) FALSE, (*frequency*) IR.Always);
