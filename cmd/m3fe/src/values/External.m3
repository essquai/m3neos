...
IMPORT Type, Expr, Variable, Ident, Scanner, RunTyme, IR, Host;
...
PROCEDURE GenLinkInfo (s: Set) =
  BEGIN
    GenInitLinks (s.exports, FALSE, IR.Export_unit);
    GenInitLinks (s.imports, TRUE,  IR.Import_unit);
  END GenLinkInfo;

PROCEDURE GenInitLinks (p: Port;  imported: BOOLEAN;
                        note: PROCEDURE (n: IR.Name)) =
  VAR x, y: Port;
  BEGIN
    x := p;
    WHILE (x # NIL) DO
