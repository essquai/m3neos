...
IMPORT M3, M3ID, IR, Value, ValueRep, Type, Scope, Error, Host;
...
VAR
  cg_procs: T := NIL;
  (* procedures that have M3IR attachments *)
...
PROCEDURE Reset () =
  ...
  BEGIN
    ...
    (* release any M3IR objects that were created. *)
    t := cg_procs;
    WHILE (t # NIL) DO
...
PROCEDURE LoadStaticLink (t: T) =
 BEGIN
    ...
    Value.Declare (t);
    IF IsNested (t)
      THEN IR.Load_static_link (t.cg_proc);
      ELSE IR.Load_nil ();
    END;
 END LoadStaticLink;

PROCEDURE ImportProc (p: T;  name: TEXT;  n_formals: INTEGER;
                      cg_result: IR.Type; return_typeid: IR.TypeUID;
  ...
  BEGIN
    ...
    p.cg_proc := IR.Import_procedure (id, n_formals,
...
PROCEDURE DeclareFormals (p: T;  cc: IR.CallingConvention) =
  VAR
    v       : Value.T;
...
PROCEDURE DeclareResult (result: Type.T) =
  ...
  BEGIN
    IF (result # NIL) AND ProcType.LargeResult (result) THEN
      IF (returnName = M3ID.NoID) THEN returnName := M3ID.Add ("_return"); END;
      result_id := IR.Declare_indirect (Type.GlobalUID (result));
      EVAL IR.Declare_param (returnName, Target.Address.size,
                Target.Address.align, IR.Type.Addr, result_id,
                in_memory := FALSE, up_level := FALSE, f := IR.Maybe);
    END;
  END DeclareResult;
...
PROCEDURE Declarer (p: T): BOOLEAN =
  ...
  BEGIN
    ...
    IF cg_result # IR.Type.Void THEN
    ...
    p.next_cg_proc := cg_procs;  cg_procs := p;
    p.cg_proc := IR.Declare_procedure (M3ID.Add (name),
...
PROCEDURE LangInit (t: T) =
  ...
  BEGIN
    ...
    ELSIF (Host.inline_nested_procs) THEN
      GenBody (t);
    ELSE
      IR.Note_procedure_origin (t.cg_proc);
    END;
  END LangInit;
...
PROCEDURE GenBody (p: T) =
  ...
  BEGIN
    ...
    IR.End_procedure (p.cg_proc);

    Scope.Pop (zz);
  END GenBody;

PROCEDURE StartRaises (t: T;  VAR(*OUT*)l: IR.Label; VAR(*OUT*)frame: IR.Var) =
  VAR raises: ESet.T;  eset: IR.Var;  eoffset: INTEGER;  frame_size : INTEGER;
  BEGIN
    ...
    IF frame_size # 0 THEN
      frame := IR.Declare_local (M3ID.NoID, frame_size, Target.Address.align,
    ...
    ELSE
      ...
      IR.Load_addr_of (eset, eoffset, Target.Address.align);
      IR.Store_addr (frame, M3RT.EF3_raises);
      Marker.PushFrame (frame, M3RT.HandlerClass.Raises);
    END;
  END StartRaises;

PROCEDURE EndRaises (t: T;  l: IR.Label;  frame: IR.Var;  fallThru: BOOLEAN) =
  VAR raises: ESet.T;
  BEGIN
    IF (NOT Host.doRaisesChk) OR (NOT t.needs_raises) THEN RETURN END;
    raises := ProcType.Raises (t.signature);
...
PROCEDURE StartCall (t: T) =
  ...
  BEGIN
    ...
    t.used := TRUE;
    Value.Declare (t);
    IR.Start_call_direct (t.cg_proc, StaticLevel (t), result);
  END StartCall;
