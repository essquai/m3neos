...
IMPORT M3, M3ID, IR, Expr, Type, TypeRep, Value, Scope, Target;
...
PROCEDURE ParseSignature (name: M3ID.T;  cc: IR.CallingConvention): Type.T =
  TYPE  TK = Token.T;
  VAR   p: P := NIL;
...
PROCEDURE Compiler (p: P) =
  VAR n_formals, n_raises: INTEGER;  v: Value.T;  result_id: IR.TypeUID;
  BEGIN
    IF (p.result = NIL) THEN
      result_id := 0;
    ELSE
    ...
    IR.Declare_proctype (Type.GlobalUID (p), n_formals, result_id, n_raises,
...
PROCEDURE ResultTypename (t: Type.T): M3ID.T =
  ...
  BEGIN
    ...
    IF p # NIL AND CGResult (p) # IR.Type.Void
...
PROCEDURE CGResult (t: Type.T): IR.Type =
  VAR p := Reduce (t);
  BEGIN
    IF (p = NIL) OR (p.result = NIL) THEN
      RETURN IR.Type.Void;
    ELSIF NOT LargeResult (p.result) THEN
      RETURN Type.IRType (p.result, in_memory := TRUE);
...
PROCEDURE CallConv  (t: Type.T): IR.CallingConvention =
  VAR p := Reduce (t);
  BEGIN
    IF (p # NIL)
