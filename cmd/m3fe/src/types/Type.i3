...
TYPE
  Info = RECORD
    ...
    (* ^When stk_type = IR.Type.Addr, the alignment of dereferenced location. *)
    hash      : INTEGER;  (* internal hash code *)
    name      := M3ID.NoID;
    stk_type  : IR.Type;  (* code generator representation on operator stack *)
    mem_type  : IR.Type;  (* code generator representation as a variable *)
    class     : Class;
    isTraced  : M3.Flag;
    isEmpty   : M3.Flag;
    isSolid   : M3.Flag;
...
PROCEDURE IRType (t: T;  in_memory: BOOLEAN := FALSE): IR.Type;
...
(* If 't' is not a structured type, generate code to load the scalar
   pointed to by the address on the IR stack *)
