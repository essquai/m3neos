...
IMPORT M3, M3ID, IR, Token, Type, TypeRep, Scanner, ObjectType, Target;
...
PROCEDURE Compiler (p: P) =
  ...
  BEGIN
    ...
    END;
    IR.Declare_pointer (typeid, Type.GlobalUID (p.target),
                        Brand.ToText (p.brand), p.isTraced, target_typename);
    IF user_name # NIL THEN
      IR.Declare_typename (typeid, target_typename);
    END;
  END Compiler;
...
(* FIXME: This is a quick hack for open arrays of element bitsize 1, 2, or 4.
    ...
    IR.Init_intt (offset + M3RT.TC_kind, 8, ORD (Kind[dims > 0]), FALSE);
    IR.Init_intt
   ...
   (* ^VSee comments in RT0.i3, regarding dataAlignment and dataSize. *)
    ...
    IF (dims > 0) THEN
      ...
      IR.Init_intt (offset + M3RT.ATC_nDimensions, isz, dims, FALSE);
      IR.Init_intt
...
PROCEDURE GenInitProc (p: P): IR.Proc =
  VAR name: TEXT;  proc: IR.Proc;  ref: IR.Var;
  BEGIN
    ...
    IR.Comment (-1, FALSE, name & " (RefType)");
