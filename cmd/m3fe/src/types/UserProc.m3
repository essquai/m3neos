...
IMPORT M3ID, IR, Type, Expr, ExprRep, ProcType, Formal;
...
PROCEDURE InnerPrep (ce: CallExpr.T) =
  ...
  BEGIN
    ...
    ELSIF CouldBeClosure (proc) THEN
      Expr.Compile (proc);
      p_temp := IR.Pop_temp ();
      IR.Start_call_indirect (cg_result, callConv);
    ELSE
      Expr.Compile (proc);
      p_temp := IR.Pop ();
      IR.Start_call_indirect (cg_result, callConv);
    ...
    IF (large_result) THEN
      ...
      IF ce.doDirectAssign THEN
        ce.tmp := lhs_result;
      ELSE
        IR.Load_addr_of_temp (tmp_result, 0, align_result);
        ce.tmp := IR.Pop ();
      END;
    END;
  END InnerPrep;
...
PROCEDURE Compile (ce: CallExpr.T) =
  BEGIN
    ...
    IF (ce.tmp # NIL) THEN
      IR.Push (ce.tmp);
      IR.Boost_addr_alignment (ce.align);
      IR.Free (ce.tmp);
      ce.tmp := NIL;
    END;
  END Compile;
...
PROCEDURE GenResultArg (lhs: IR.Val;  tmp: IR.Var;  align: IR.Alignment) =
  BEGIN
    IF (lhs # NIL)
      THEN IR.Push (lhs);
      ELSE IR.Load_addr_of (tmp, 0, align);
    END;
...
PROCEDURE GenClosureCall (p_temp: IR.Val;  result: IR.Type;
  ...
  VAR skip := IR.Next_label ();
      ...
      info : IR.Var;
  BEGIN
    IR.If_closure (p_temp, IR.No_label, skip, IR.Maybe);
    IR.Push (p_temp);
    IR.Closure_frame ();
    IR.Pop_static_link ();
    IR.Push (p_temp);
    IR.Closure_proc ();
    IR.Store_temp (p_temp);
    IR.Set_label (skip);
    IR.Push (p_temp);
    IF Marker.NextHandler(handler, handler_body, info) THEN
