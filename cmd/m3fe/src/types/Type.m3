...
IMPORT IR, Error, Token, Scanner, NamedType, Word;
...
(*EXPORTED*)
PROCEDURE IRType (t: T;  in_memory: BOOLEAN): IR.Type =
  BEGIN
    t := Check (t);
    IF (in_memory)
...
(* Always represented as an address on the IR stack (record, array, or large set) *)
...
PROCEDURE GenTag (t: T;  tag: TEXT;  offset: INTEGER) =
  BEGIN
    ...
    IR.Comment (offset, FALSE, tag, Name (t));
  END GenTag;
...
PROCEDURE GenCells (): INTEGER =
  ...
  BEGIN
    ...
    WHILE (cell # NIL) DO
      IR.Comment (cell.offset, FALSE, "typecell for ", Name (cell.type));
...
PROCEDURE GenCellPtrs (): INTEGER =
  ...
  BEGIN
    ...
    IF (cell = NIL) THEN RETURN -1; END;
    WHILE (cell # NIL) DO
      IF (prev # 0) THEN IR.Init_var (cell.offset, unit, prev, FALSE); END;
      IR.Init_intt (cell.offset + Target.Address.pack, Target.Integer.size,
  ...
  BEGIN
    ...
    IF (NOT zeroed) THEN
      ...
      IF (c2 = 0) THEN
        ...
        IR.Boost_addr_alignment (t.info.alignment);
        Zero (t);
        RETURN;
      ELSIF (c1 > 2 * c2) THEN
...
PROCEDURE Zero (full_t: T) =
  ...
  BEGIN
    ...
    | Class.Integer, Class.Longint, Class.Subrange, Class.Enum =>
        IR.Load_integer (u.info.stk_type, TInt.Zero);
        IR.Store_indirect (u.info.stk_type, 0, size);
    | Class.Real =>
        IR.Load_float (TFloat.ZeroR);
        IR.Store_indirect (IR.Type.Reel, 0, size);
    | Class.Longreal =>
        IR.Load_float (TFloat.ZeroL);
        IR.Store_indirect (IR.Type.LReel, 0, size);
    | Class.Extended =>
        IR.Load_float (TFloat.ZeroX);
        IR.Store_indirect (IR.Type.XReel, 0, size);
    | Class.Opaque, Class.Ref, Class.Object, Class.Procedure =>
        IR.Load_nil ();
        IR.Store_indirect (IR.Type.Addr, 0, size);
    | Class.Set =>
        IF (size <= Target.Integer.size) THEN
          IR.Load_integer (Target.Integer.cg_type, TInt.Zero);
          IR.Store_indirect (Target.Integer.cg_type, 0, size);
...
PROCEDURE ZeroWords (size: INTEGER) =
  VAR lv := IR.Pop ();  offset := 0;
  BEGIN
    WHILE (size >= Target.Integer.size) DO
      IR.Push (lv);
      IR.Load_integer (Target.Integer.cg_type, TInt.Zero);
      IR.Store_indirect (Target.Integer.cg_type, offset, Target.Integer.size);
