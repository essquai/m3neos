...
IMPORT M3, IR, Type, TypeRep, Error, Target, TInt, Word;
...
(* EXPORTED: *)
PROCEDURE DeclareDopeTemp (t: Type.T): IR.Var =
...
PROCEDURE GenInit (p: P;  zeroed: BOOLEAN) =
  ...
  BEGIN
    ...
    IR.Push (array);
    IR.Open_elt_ptr (ArrayType.EltAlign (p));
    IR.Free (array);
    array := IR.Pop ();
    ...
    IR.Push (array);
    IR.Push (cnt);
    IR.Index_bytes (p.NonopenEltPack);
