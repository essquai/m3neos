...
IMPORT M3, IR, Type, TypeRep, Int, LInt, Expr, Token, Card, M3Buf;
...
PROCEDURE Compiler (p: P) =
  BEGIN
    ...
    IR.Declare_subrange (Type.GlobalUID (p), Type.GlobalUID (p.baseType),
...
PROCEDURE InitCoster (p: P;  zeroed: BOOLEAN): INTEGER =
  ...
  BEGIN
    ...
    CASE p.rep OF
    | IR.Type.Word8  => rep_min := Target.Word8.min;  rep_max := Target.Word8.max;
    | IR.Type.Word16 => rep_min := Target.Word16.min; rep_max := Target.Word16.max;
    | IR.Type.Word32 => rep_min := Target.Word32.min; rep_max := Target.Word32.max;
    | IR.Type.Word64 => rep_min := Target.Word64.min; rep_max := Target.Word64.max;
    | IR.Type.Int8   => rep_min := Target.Int8.min;   rep_max := Target.Int8.max;
    | IR.Type.Int16  => rep_min := Target.Int16.min;  rep_max := Target.Int16.max;
    | IR.Type.Int32  => rep_min := Target.Int32.min;  rep_max := Target.Int32.max;
    | IR.Type.Int64  => rep_min := Target.Int64.min;  rep_max := Target.Int64.max;
...
PROCEDURE GenInit (p: P;  zeroed: BOOLEAN) =
  ...
  BEGIN
    ...
    IF TInt.LT (TInt.Zero, p.min) OR TInt.LT (p.max, TInt.Zero) THEN
      IR.Load_integer (info.stk_type, p.min);
      IR.Store_indirect (info.stk_type, 0, info.size);
    ELSIF zeroed THEN
      IR.Discard (IR.Type.Addr);
    ELSE
      IR.Load_integer (info.stk_type, TInt.Zero);
      IR.Store_indirect (info.stk_type, 0, info.size);
    END;
  END GenInit;
